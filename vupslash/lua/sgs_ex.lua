...
function sgs.CreateTriggerSkill(spec)
    ...
    local skill = sgs.LuaTriggerSkill(spec.name, frequency, limit_mark, change_skill, limited_skill, hide_skill)
...
function sgs.CreateSkillCard(spec)
	...
	if type(spec.no_indicator) == "boolean" then
		card:setIndicatorHide(spec.no_indicator)
	end
...
function sgs.CreateEquipCard(spec)
	...
	if spec.location == sgs.EquipCard_DefensiveHorseLocation or spec.location == sgs.EquipCard_OffensiveHorseLocation then assert(type(spec.correct) == "number") end
	...
	elseif spec.location == sgs.EquipCard_OffensiveHorseLocation then
		card = sgs.LuaOffensiveHorse(spec.suit or sgs.Card_NoSuit, spec.number or 0, spec.correct, spec.name, spec.class_name)
	elseif spec.location == sgs.EquipCard_DefensiveHorseLocation then
		card = sgs.LuaDefensiveHorse(spec.suit or sgs.Card_NoSuit, spec.number or 0, spec.correct, spec.name, spec.class_name)
	end
	assert(card ~= nil)
