...
function SmartAI:AtomDamageCount(target, source, nature, card)
	...
	elseif nature == sgs.DamageStruct_Thunder then --如果是雷电伤害
		if target:hasArmorEffect("toujing") then --如果目标装备了透镜
			count = count + 1
			self.room:writeToConsole("toujing+1")
		end
	end
...
function SmartAI:DamageToCards(target, source, count)
	...
	if target then --伤害目标必须存在
		...
		--竭心（按次数结算）
		if target:hasSkill("jiexin") then
			thisRangeMax = thisRangeMax + 1.5
			thisRangeMin = thisRangeMin + 1.5
		end
		--心动（按次数结算）
		if target:hasSkill("xindong") then
			if source then
				if not civil then
					thisRangeMax = thisRangeMax + 1
					thisRangeMin = thisRangeMin + 1
					...
					thisRangeMax = thisRangeMax + 2
					thisRangeMin = thisRangeMin + 2
				end
			end
		end
	end
...
function SmartAI:ImitateResult_DrawNCards(player, skills, overall)
	...
	if #drawSkills > 0 then
		...
		for _,skillname in pairs(drawSkills) do
			...
			elseif skillname == "chouka" then	--抽卡，视为多摸一张牌
				count = count + 1
			elseif skillname == "yishou" then	--忆狩，多摸忆狩叠加层数×2张牌
				count = count + 2*player:getMark("yishou_active")
			elseif skillname == "luafenyin" and player:getMark("&luafenyin!") > 0 then	--奋音（新），多摸一张牌
				count = count + 1
			end
		end
	end
