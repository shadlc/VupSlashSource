...
function SmartAI:doNotSave(player)	--不救人
	if (player:hasSkill("niepan") and player:getMark("@nirvana") > 0 and player:getCards("e"):length() < 2)
		--or (player:hasSkill("shengguangN") and player:getMark("@halo") > 0)	--圣光用了之后体力上限就为0了，还是要救不然没机会了
...
sgs.ai_skill_use_func.ShenfenCard = function(card, use, self)
	...
	if not self.player:faceUp() or self.player:hasSkills("jushou|ol_jushou|nosjushou|neojushou|kuiwei") then
		benefit = benefit + 1
	else
...
--------------------------------------------------
--需要忍标记，现已扩展为需要空过/弃牌
--第二个新参数为真则不能用任何牌，不为真则可以用无中、顺之类的不会使手牌数减少的牌
--第三个参数代表固定一个overflow值（防止无限引用循环）
--第四个新参数代表被排除的技能
--------------------------------------------------

function SmartAI:needBear(player, Not_use_any, overflow, except_skills)
	player = player or self.player
	overflow = overflow or self:getOverflow(player)
	except_skills = except_skills or ""
	local excepts = {}
	excepts = except_skills:split("|")
	
	if not Not_use_any then		--可以用不会使手牌数减少的牌
		
		--郁生（尽量让手牌数=体力值，但喝了酒还是要出杀）
		if player:hasSkill("yusheng") and not table.contains(excepts, "yusheng") and player:getPhase() == sgs.Player_Play and player:getHandcardNum() == player:getHp() and player:getMark("drank") == 0 then
			return true
		end
		
		--心动（需要回合内弃牌数）
		if player:hasSkill("xindong") and not table.contains(excepts, "xindong") and player:getPhase() ~= sgs.Player_NotActive and player:getMark("xindong_count") < 2 and player:getMark("xindong_count") + overflow == 2 and player:getMark("drank") == 0 then
			local targets = {}
			...
			local target = self:findPlayerToDamage(1, player, sgs.DamageStruct_Normal, targets, false, 20, false)
			if target then
				return true
			end
		end
		...
		--渊回（符合条件就直接结束）
		if player:hasSkill("yuanhui") and not table.contains(excepts, "yuanhui") and player:getPhase() == sgs.Player_Play and overflow <= 0 and player:getMark("yuanhui_red") + player:getMark("yuanhui_black") == 1 then
			local can_trigger = false
			for _,p in sgs.qlist(self.room:getOtherPlayers(player)) do
				if self:canDamage(p, player, false, sgs.DamageStruct_Normal) and p:getMark("yuanhui_red") + p:getMark("yuanhui_black") == 1 and player:getMark("yuanhui_red") + p:getMark("yuanhui_red") == 1 and player:getMark("yuanhui_black") + p:getMark("yuanhui_black") == 1 then
					can_trigger = true
					break
				end
			end
		...
		--占梦（出牌阶段至少留一张，不溢出留两张，但喝了酒还是要出杀）
		if player:hasSkill("zhanmeng") and not table.contains(excepts, "zhanmeng") and player:getPhase() == sgs.Player_Play and (player:getHandcardNum() == 1 or (player:getHandcardNum() == 2 and overflow <= 0)) and player:getMark("drank") == 0 then
			return true
		end
		
		--奉声（新），同占梦
		if player:hasSkill("newfengsheng") and not table.contains(excepts, "newfengsheng") and player:getPhase() == sgs.Player_Play and (player:getHandcardNum() == 1 or (player:getHandcardNum() == 2 and overflow <= 0)) and player:getMark("drank") == 0 then
			return true
		end
		
		--布教（血量危险时需要主动弃牌，但喝了酒还是要出杀）
		if player:hasSkill("bujiao") and not table.contains(excepts, "bujiao") and player:getPhase() == sgs.Player_Play and self.player:getHp() <= 3 and overflow == 1 and player:getMark("drank") == 0 then
			return true
		end
		
		--争欲（只打溢出牌，但喝了酒还是要出杀）
		if player:hasSkill("zhengyu") and not table.contains(excepts, "zhengyu") and player:getPhase() == sgs.Player_Play and overflow <= 0 and player:getMark("drank") == 0 then
			return true
		end
		
		--沉梦（不溢出则起码留2张手牌，但喝了酒还是要出杀）
		if player:hasSkill("chenmeng") and not table.contains(excepts, "chenmeng") and player:getPhase() == sgs.Player_Play and overflow <= 0 and self.player:getHandcardNum() <= 2 and player:getMark("drank") == 0 then
			return true
		end
		
		--鼠化（没觉醒就尽快觉醒，但喝了酒还是要出杀）
		if player:hasSkill("shuhua") and not table.contains(excepts, "shuhua") and player:hasSkill("qiaohui") and player:getMark("qiaohui") == 0 and player:getPhase() == sgs.Player_Play and player:getMark("drank") == 0 then
			local count = 0
			for _,p in sgs.qlist(self.room:getAlivePlayers()) do
		...
		--丰收（适合开五谷就弃一张）
		if player:hasSkill("fengshou") and not table.contains(excepts, "fengshou") and player:getPhase() == sgs.Player_Play and overflow == 1 and self:willUseAmazingGrace(amazing_grace) then
			amazing_grace:deleteLater()
			return true
		end
		amazing_grace:deleteLater()
		
		--玄鳞（不溢出且装备区牌数≤2张就过，但喝了酒还是要出杀）
		if player:hasSkill("xuanlin") and not table.contains(excepts, "xuanlin") and player:getPhase() == sgs.Player_Play and overflow <= 0 and self.player:getEquips():length() <= 2 and player:getMark("drank") == 0 then
			return true
		end
		
		--酣歌冰火（有回血需求就弃两张）
		if player:hasSkill("hange_if") and (overflow == 2 or overflow <= 0) then
			self:sort(self.friends, "defense")
			for _, p in ipairs(self.friends) do
				if p:getLostHp() > 0 and not self:needToLoseHp(p, player, false, false, true) then
					return true
				end
			end
		end
		
		--奇遇（出牌阶段至少留一张，但喝了酒还是要出杀）
		if player:hasSkill("qiyu") and not table.contains(excepts, "qiyu") and player:getPhase() == sgs.Player_Play and player:getHandcardNum() == 1 and player:getMark("drank") == 0 then
			return true
		end
		...
		--
		----勤国（技能攒齐之前每回合弃两张）
		--if player:hasSkill("luaqinguo") and (overflow == 2 or overflow <= 0) and not player:hasSkills("luazhiyan+luapojun+luajiang") then
		--	return true
		--end
		--
		----秉壹（出牌阶段留一张）
		--if player:hasSkill("luabingyi") and player:getPhase() == sgs.Player_Play and player:getHandcardNum() == 1 then
		--	return true
		--end
		--
		----界权计
		--if player:hasSkills("luaquanji_jie+zili") and player:getPile("power"):length() < 3 and ((not player:hasSkill("paiyi") and overflow == 1) or (player:hasSkill("paiyi") and (player:getHandcardNum() == player:getHp() + 1))) then
		--	return true
		--end
		--
		----界屯田（回合内弃一张，回合外不影响）
		--if player:hasSkills("luatuntian_jie|luatuntian_jieh") and player:getPhase() ~= sgs.Player_NotActive then
		--	if player:hasSkill("zaoxian") and not player:hasSkill("jixi") and player:getPile("field"):length() < 3 and overflow <= 1 then	--没觉醒凿险，无脑屯
		--		return true
		--	end
		--	if player:hasSkills("jixi|luafengchan") and player:getPile("field"):length() < math.max(3, self.player:aliveCount()) and overflow == 1 and player:getMark("drank") == 0 then
		--		return true
		--	end
		--	if player:getPile("field"):length() == 0 and overflow <= 1 then
		--		return true
		--	end
		--end
		--
		----神智（不溢出就蓄非基本）
		--if player:hasSkill("luashenzhi_x") and player:getPhase() == sgs.Player_Play and overflow <= 0 then
		--	for _, c in sgs.qlist(player:getHandcards()) do
		...
		--
		----界慎断（没断时弃牌攒断）
		--if player:hasSkill("luashenduan") and player:getPhase() == sgs.Player_Play and player:getPile("duan"):length() == 0 and overflow > 0 then
		--	local count = 0
		--	for _, c in sgs.qlist(player:getHandcards()) do
		--		if (c:isKindOf("BasicCard") or c:isKindOf("EquipCard")) and c:isBlack() then
		--			count = count + 1
		--		end
		--	end
		...
		--
		----征荣（蓄单张征荣牌）
		--if player:hasSkill("luazhengrong") and player:getPhase() == sgs.Player_Play and math.mod(player:getMark("luazhengrong_count"), 2) == 0 and overflow <= 0 then
		...
		--		end
		--	end
		--	if zhengrong_count == 1 then
		--		return true
		--	end
		--end
		--
		----利盟（出牌阶段留一张）
		--if player:hasSkill("lualimeng") and player:getPhase() == sgs.Player_Play and player:getHandcardNum() == 1 then
		--	return true
		--end
		
		--翊赞→全能（不溢出、有濒危队友、有使用次数就蓄基本）
		if player:hasSkill("quanneng") and not table.contains(excepts, "quanneng") and player:getMark("quanneng_used") == 0 then
			local friend_need_peach = 0
			local friend_num = 0
			for _, friend in ipairs(self:getFriends(player)) do
				if self:isWeak(friend) or friend:getHp() <= 1 then
					friend_need_peach = friend_need_peach + 1
				end
				friend_num = friend_num + 1
			end
			local has_friend_to_save = false
			if friend_num > 1 and friend_need_peach > 0 then
				has_friend_to_save = true
			end
			if player:getPhase() == sgs.Player_Play and overflow <= 0 then
				local count = 0
				for _, c in sgs.qlist(player:getHandcards()) do
				...
				if (count == 1 and player:getMark("drank") == 0) or (count > 1 and self:hasCrossbowEffect(player)) or (player:getHp() <= 2) then
					return (count == 1 or player:getHp() <= 2) or has_friend_to_save
				end
			elseif player:getPhase() == sgs.Player_NotActive then
				...
				if hasBasic and (self:getOverflow(player) <= 0) and self.room:getCurrent():getMark("drank") == 0 then
					return true
				end
			end
		end
		
		--紫禁（不溢出、这回合未造成过伤害就蓄非基本）
		if player:hasSkills("zijin") and not table.contains(excepts, "zijin") and not player:hasFlag("damage_caused_ruqin") and player:getMark("drank") == 0 then
			if player:getPhase() == sgs.Player_Play and overflow <= 0 then
				local count = 0
				for _, c in sgs.qlist(player:getHandcards()) do
		...
		--巨贾（AI决定要回血时不溢出用牌）
		--if player:hasSkill("luajugu") and player:hasFlag("luajugu_AI_needBear") and player:getPhase() == sgs.Player_Play and overflow <= 0 then
		--	return true
		--end
	else		--一张牌都不能用
