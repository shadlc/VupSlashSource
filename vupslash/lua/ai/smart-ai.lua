...
function setInitialTables()
    ...
    sgs.need_kongcheng = "lianying|noslianying|lualianying|kongcheng|sijian|hengzheng" ..
									"|guatian|hengxin|liucai"
    ...
    sgs.wizard_harm_skill =     "guicai|guidao|jilve" ..
										"|luadongxiao|tonggan|weiliu"
    sgs.priority_skill =        "dimeng|haoshi|qingnang|nosjizhi|jizhi|guzheng|qixi|jieyin|guose|duanliang|ol_duanliang|jujian|fanjian|neofanjian|lijian" ..
    ...
    sgs.save_skill =        "jijiu|buyi|nosjiefan|chunlao|longhun" ..
										"|luazuijiao|luazhafu|yizan"..
										"|yanling|quanneng|shisui|fuyin_shayue"
    sgs.exclusive_skill =       "huilei|duanchang|wuhun|buqu|dushi|zhuiyi|yanhuo|juexiang" ..
											"|chunni|qiji|yuguang|xingji"
    sgs.Active_cardneed_skill =     "paoxiao|luapaoxiao|tianyi|xianzhen|shuangxiong|nosjizhi|jizhi|guose|duanliang|ol_duanliang|qixi|qingnang|luoyi" ..
                                                "|guhuo|nosguhuo|jieyin|zhiheng|rende|nosrende|nosjujian|luanji|mobileluanji|qiaobian|lirang|mingce"..
                                                "|fuhun|spzhenwei|nosfuhun|nosluoyi|yinbing|jieyue|sanyao|xinzhan"..
												"|luazhiheng|luayayin|luaaotu|luahuoji"..
												"|libeng|zhuoshi|xianwei|yinmou|jinzhou|jingsuan|shuoyi|yuejian|xingxiong|bianshi|jiyue|lianjie|mingguang|xixue|chenmeng|zhanshu|motiao|newtongwo|wuji_buding|qisheng"
    sgs.notActive_cardneed_skill =      "kanpo|guicai|guidao|beige|xiaoguo|liuli|tianxiang|jijiu|leiji|nosleiji|olleiji|niluan"..
                                                    "|qingjian|zhuhai|qinxue|jspdanqi"..
													"|luacidui|lualanghao|luakanpo|luashenzhi_x|luayingshi|luadongxiao|yizan|luaposhi"..
													"|quanneng|yunyao|xingyi|fengjin|kuqing|bingjiao|zhengyu|guizhou|guanxi|weiliu|yangneng|wenyu|newfengsheng"
    sgs.cardneed_skill =  sgs.Active_cardneed_skill .. "|" .. sgs.notActive_cardneed_skill
    sgs.drawpeach_skill =       "tuxi|qiaobian" ..
									"|conggong|cangbao|chongya"
    sgs.recover_skill =     "nosrende|rende|kofkuanggu|kuanggu|ol_kuanggu|zaiqi|jieyin|qingnang|yinghun|hunzi|shenzhi|longhun|nosmiji|zishou|ganlu|xueji|shangshi" ..
                        "|nosshangshi|ytchengxiang|buqu|miji|quji" ..
                        "|luashenzhi_x|shitian|quanneng|jieshuo|yangge|qianchang|jixue|juling"
    ...
    sgs.judge_reason =      "bazhen|EightDiagram|wuhun|supply_shortage|tuntian|nosqianxi|nosmiji|indulgence|lightning|baonue"..
                                    "|nosleiji|leiji|caizhaoji_hujia|tieji|luoshen|ganglie|luaganglie|neoganglie|vsganglie|kofkuanggu"..
...
function sgs.getDefense(player)
    ...
    if player:hasTreasure("yuhangtu") then defense = defense + 2 end

    local hasEightDiagram = false
    if player:hasArmorEffect("eight_diagram") or (player:hasSkills("bazhen|xiangrui") and not player:getArmor()) then
        hasEightDiagram = true
    end
    if hasEightDiagram then
        if player:hasSkills("tiandu|luatiandu") then defense = defense + 1 end
        ...
        if player:hasSkill("xiaoan") then defense = defense + 2 end
    ...
    if player:hasSkill("lualuohong") then defense = defense + 2 end
    if player:getMark("@tied") > 0 then defense = defense + 1 end
    if player:hasSkill("chengxiang") then defense = defense + 2 end
    if player:hasSkill("zhongzuo") then defense = defense + 0.5 end
    if player:hasSkills("xianrou|geiwoxinxin|lianmei") then defense = defense + 2 end
    if player:hasSkills("yinqi") and player:isKongcheng() then defense = defense + 2 end
    if player:hasSkills("qingling") and player:getHp() > 1 then defense = defense + 2 end
    if player:hasSkills("chouzhen") and player:getChangeSkillState("chouzhen") == 2 then defense = defense - 2 end	--筹赈②为负防御
    if player:hasSkills("hualin") and player:getMark("&hualin+used+_lun") == 0 then	--化鳞，场上有黑桃且没发动过防御力增加
    ...
    if player:hasSkills("kuanggu|ol_kuanggu") and player:getHp() > 1 then defense = defense + 0.5 end
    ...
    if player:hasSkills("wanrong|kangkai|luashouye") and not hasManjuanEffect(player) then defense = defense + 0.5 end
    if player:hasSkill("luabijing") then
		if player:getMark("luabijing_used") == 0 and not hasManjuanEffect(player) then
			defense = defense + 0.5
		elseif player:getMark("luabijing_used") > 0 then
			defense = defense - 0.5
		end
	end
    if player:hasSkill("tianxiang") then defense = defense + player:getHandcardNum() * 0.5 end
    if player:hasSkill("luacidui") then defense = defense + player:getHandcardNum() * 0.5 end
    if player:hasSkill("andong") then defense = defense + 1 end
    ...
    if player:hasSkill("yizan") then defense = defense + 2 end
    if player:hasSkill("quanneng") then defense = defense + 2 end
...
function SmartAI:assignKeep(start)
	...
	if not self:isWeak() and self.player:hasSkills("luashenzhi_x") then
		self.keepdata.EquipCard = (self.keepdata.Jink or 5.2) + 0.1
		self.keepdata.TrickCard = (self.keepdata.Jink or 5.2) + 0.1
	end
...
function SmartAI:getKeepValue(card, kept, writeMode)
    ...
    if writeMode then
        if cardPlace == sgs.Player_PlaceEquip then
            ...
            elseif self.player:hasSkills("bazhen|xiangrui|yizhong|bingshen") and card:isKindOf("Armor") then return -8
			...
			if self:isTemporaryCard(self.player, card:getEffectiveId()) then
				value = value - 9
			end
        ...
        elseif cardPlace == sgs.Player_PlaceHand then
			...
			local value_fix = 0
			if self:isTemporaryCard(self.player, card:getEffectiveId()) then
				value_fix = value_fix - 9
			end

            newvalue = maxvalue + value_suit + value_number + value_fix
...
function SmartAI:adjustKeepValue(card, v)
	...
	if self.player:getMaxHp() <= 0 and (card:isKindOf("Peach") or card:isKindOf("Analeptic")) then	--已经没有体力上限了，对桃酒的处理
		if (#self:getFriendsNoself(self.player) == 0 or card:isKindOf("Analeptic") and self.player:getMaxCards() == 1) then	--没队友不留桃酒，有队友但自己手牌上限为1不留酒
			v = -10
		end
		v = v - 2	--将桃的保留优先级调整至第一张闪之后
	end
    ...
    if card:isKindOf("Slash") then
        ...
        if self.player:hasSkills("jiang|luajiang") and card:isRed() then v = v + 0.004 end
        if self.player:hasSkill("wushen") and card:getSuit() == sgs.Card_Heart then v = v + 0.003 end
        if self.player:hasSkill("luawushen") and card:getSuit() == sgs.Card_Heart then v = v + 0.01 end
        ...
        if card:hasFlag("&gugu") and self.player:getLostHp() > 0 and not self:needToLoseHp(self.player, self.player, false, false, true) then v = v - 0.1 end
        if self.player:hasSkills("qilv") and not self:hasCrossbowEffect() then v = -10 end
        if self.player:hasSkills("guatian") then v = -10 end
        if self.player:hasSkills("zhuge") and not self:hasCrossbowEffect() then v = -5 end
    end

	if (card:isKindOf("BasicCard") or card:isKindOf("EquipCard")) and card:isBlack() and self.player:hasSkill("shenduan|luashenduan") then
		v = -10
	end

	if card:isKindOf("BasicCard") and card:isRed() and self.player:hasSkill("fagun") and self.player:getPhase() ~= sgs.Player_NotActive and self.player:getMark("@fagun_effect") == 0 then
		v = -10
	end

	if self.player:hasSkill("yuanhui") and self.player:getPhase() ~= sgs.Player_NotActive and self.player:getMark("yuanhui_red") + self.player:getMark("yuanhui_black") == 1 then
		if card:isBlack() and self.player:getMark("yuanhui_black") == 1 then
			v = -10
		elseif card:isRed() and self.player:getMark("yuanhui_red") == 1 then
			v = -10
		end
	end

	if card:hasFlag("&migao") then
		for _, p in sgs.qlist(self.room:findPlayersBySkillName("mishi")) do	--蜜糕牌，检查场上是否有未觉醒的迷食，有则保留（keep value调整至高于第二张桃），与其敌对则反之
			if p:getMark("mishi") == 0 then
				if self:isFriend(p, self.player) then
					v = 6
				elseif self:isEnemy(p, self.player) then
					v = -10
				end
				break
			end
	...
	if card:isKindOf("Weapon") and self.player:hasSkills("jvhe") then		--居合，留一张武器（优先级2.08→4.08，第二张闪优先级为4）
		v = v + 2
	end
	
	if card:isKindOf("Slash") and self.player:hasSkills("guanxi") and not self:isWeak(self.player, true) then		--贯袭，自己不濒危则留杀
		v = v + 1
	end
	
	if card:hasFlag("&gugu") and self.player:getLostHp() > 0 and not self:needToLoseHp(self.player, self.player, false, false, true) then	--咕牌，需要回血就扔
		v = -10
	end
	
	if card:isKindOf("EquipCard") and self.player:hasSkills("weiliu") then		--维留，存装备（优先级2.08→4.08，第二张闪优先级为4）
		v = v + 2
	end
	
	if self.player:hasSkills("jiangzhuang") and card:isKindOf("EquipCard") and self.room:getCardPlace(card:getEffectiveId()) == sgs.Player_Hand then	--姜撞，手牌中的装备不留（回合外也不要，反正只能回合内装）
		v = v - 10
	end
	
	if self.player:hasSkills("xuanlin") and card:isBlack() then		--玄鳞，手牌中的黑牌价值略微上升
		v = v + 0.1
	end
	
	if self.player:hasSkills("huahuan") and card:getSuit() == sgs.Card_Club then		--花环，手牌中的梅花牌价值略微上升
		v = v + 0.1
	end
	
	if self.player:hasSkills("wenyu") and card:isKindOf("Slash") then		--温御，手牌中【杀】价值上升，红杀上升更多（3.6->5.2->5.8）
		v = v + 1.7
		if card:isRed() then
			v = v + 0.8
		end
	...
	if card:isKindOf("EquipCard") and self.player:hasSkills("jingxie") then
		local max_hp = self.player:getHp()
		for _,p in sgs.qlist(self.room:getOtherPlayers(player)) do
			if p:getHp() > max_hp then
				max_hp = p:getHp()
			end
		end
		if self.player:getHp() - 1 < max_hp then
			v = v + 5.2
		end
	end

	if card:isKindOf("BasicCard") and self.player:hasSkills("yizan|quanneng") then
		v = v + 7
	end
...
function SmartAI:getUseValue(card)
	...
	local in_hand = (self.room:getCardPlace(card:getEffectiveId()) == sgs.Player_PlaceHand) and (self.room:getCardOwner(card:getEffectiveId()):objectName() == self.player:objectName())
    ...
    if card:getTypeId() == sgs.Card_TypeEquip then
        ...
        if self.player:hasSkills("luaqiangxi") and card:isKindOf("EquipCard") then v = 2 end
        if self.player:hasSkill("kurou") and card:isKindOf("Crossbow") then return 9 end
        if self.player:hasSkills("bazhen|yizhong|bingshen") and card:isKindOf("Armor") then v = 2 end
        if self.player:hasSkills("xiangrui") and card:isKindOf("Armor") then v = 0 end
        if self.role == "loyalist" and self.player:getKingdom()=="wei" and not self.player:hasSkills("bazhen|xiangrui") and getLord(self.player) and getLord(self.player):hasLordSkill("hujia") and card:isKindOf("EightDiagram") then
            v = 9
        end
        if self.player:hasSkills(sgs.lose_equip_skill) then return 10 end
        if self.player:getMark("&motiao_using") > 0 then return 10 end
		if self.player:hasSkill("luaxingzhao") then
			...
			if injured_count >= 2 then
				return 9
			end
		end
		if card:isKindOf("WoodenOx") and self.player:getPile("wooden_ox"):length() > 0 then
			return 8 + self.player:getPile("wooden_ox"):length()
		end
    elseif card:getTypeId() == sgs.Card_TypeBasic then
        if card:isKindOf("Slash") then
            ...
            if self.player:getPhase() == sgs.Player_Play and self:slashIsAvailable() and #self.enemies > 0 and in_hand and self:getCardsNum("Slash") == 1 then v = v + 5 end
            if self.player:getPhase() == sgs.Player_Play and self:slashIsAvailable() and #self.enemies > 0 and not in_hand and self:getCardsNum("Slash") == 0 then v = v + 1 end
            ...
            if card:getSkillName() == "luasuisheng" then v = v + 1 end
            if card:getSkillName() == "fenxin_S" then v = v + 1 end
            if card:getSkillName() == "jichi" then v = v + 1 end
            if card:getSkillName() == "fuhun" then v = v + (self.player:getPhase() == sgs.Player_Play and 1 or -1) end
            if card:getSkillName() == "mingguang" then v = v + 1 end
            if self.player:hasSkill("qilv") and not self:hasCrossbowEffect() and not card:isVirtualCard() and in_hand then v = v-5 end
            --if self.player:hasSkill("guatian") and not card:isVirtualCard() and in_hand then v = v-5 end
        elseif card:isKindOf("Jink") then
            if self:getCardsNum("Jink") >= 1 and not in_hand then v = v-3 end
            if self:getCardsNum("Jink") >= 2 then v = v-3 end
            if self:getCardsNum("Jink") >= 3 and in_hand then v = v-3 end
            if card:getSkillName() == "longdan" and self.player:hasSkill("chongzhen") then v = v + 1 end
            if card:getSkillName() == "luajuzhan" then v = v + 1 end
            if card:getSkillName() == "luasuisheng" then v = v + 1 end
        elseif card:isKindOf("Peach") then
            if self.player:isWounded() then v = v + 6 end
            if self:isWeak() then v = v + 1 end
			if self.player:getMark("&motiao_using") > 0 then v = v + 5 end
        elseif card:isKindOf("Analeptic") then
			if self.player:getMark("&motiao_using") > 0 then v = v + 5 end
        elseif card:inherits("Shit") and self.player:hasSkills("kuanggu|ol_kuanggu") and card:getSuit()~= sgs.Card_Spade then
            v = 0.1
        end
    elseif card:getTypeId() == sgs.Card_TypeTrick then
		...
		if card:isKindOf("Snatch") or card:isKindOf("SupplyShortage") then
			local can_use = false
			for _, enemy in ipairs(self.enemies) do
				if not self.room:isProhibited(self.player, enemy, card) and card:isAvailable(self.player) and (self.player:distanceTo(enemy) <= (self.player:hasSkill("duanliang") and 2 or 1) or (self.player:hasSkill("ol_duanliang") and self.player:getHandcardNum() <= enemy:getHandcardNum())) then
					can_use = true
					break
				end
			end
		...
		if card:isKindOf("Nullification") and not in_hand then
			v = v - 3
		end
        if card:getSkillName() == "shuangxiong" then v = 6 end
        if card:getSkillName() == "luajuzhan" then v = 6 end
        if card:isKindOf("Duel") then v = v + self:getCardsNum("Slash") * 2 end
        if self.player:hasSkill("nosjizhi|zhuoshi|qilv|zhanshu") then v = v + 4 end
        if self.player:hasSkill("jizhi") then v = v + 3 end
        if card:getSkillName() == "luahuoji" then v = v + 2 end
		if card:getSkillName() == "duanliang" and self.player:hasSkill("jiezi") then v = v + 4 end
		if card:getSkillName() == "ol_duanliang" and self.player:hasSkill("jiezi") then v = v + 4 end
		...
		if card:isKindOf("Fudichouxin") and self.player:hasSkills(sgs.lose_equip_skill.."|motiao|shuoyi|xixue|wuji_buding") and not self.player:getEquips():isEmpty() then v = v + 5 end
    end


    if self:hasSkills(sgs.need_kongcheng) then
        if self.player:getHandcardNum() == 1 and in_hand then v = 10 end
    end

    if self.player:hasSkill("canshi") and self.player:hasFlag("canshi") then v = v - 2 end
    if self.player:hasSkill("chouhai") then
        if self.player:hasFlag("canshi") and self.player:getHandcardNum() == 2 and in_hand then v = 1 end
        if self.player:getHandcardNum() == 1 and in_hand then v = 1 end
    end

	if self.player:hasSkill("luayingshi") and card:getSuit() == sgs.Card_Heart then
...
function SmartAI:adjustUsePriority(card, v)
    ...
    if card:isKindOf("Slash") then
        ...
        if card:isKindOf("NatureSlash") then
            if self.slashAvail == 1 then
                ...
                elseif card:isKindOf("ThunderSlash") then
                    for _, enemy in ipairs(self.enemies) do
                        if enemy:getMark("@fog") > 0 or enemy:hasArmorEffect("toujing") then v = v + 0.06 break end
                    end
                end
            else v = v - 0.05
            end
        ...
        if card:getSkillName() == "luasuisheng" then v = v + 0.05 end
        if card:getSkillName() == "fenxin_S" then v = v + 0.05 end
        if card:getSkillName() == "jichi" then v = v + 0.05 end
        if card:getSkillName() == "fuhun" then v = v + (self.player:getPhase() == sgs.Player_Play and 0.06 or -0.05) end
        if card:getSkillName() == "mingguang" then v = v + 0.06 end
        if self.player:hasSkills("jiang|luajiang") and card:isRed() then v = v + 0.05 end
        if self.player:hasSkills("shenduan|luashenduan") and card:isBlack() and card:isKindOf("BasicCard") then v = v - 0.05 end
		if self.player:hasSkill("fagun") and card:isRed() and card:isKindOf("BasicCard") and self.player:getPhase() ~= sgs.Player_NotActive and self.player:getMark("@fagun_effect") == 0 then v = v - 0.05 end
        if self.player:hasSkill("luaaotu") and card:isKindOf("Analeptic") then v = v + 3 end
        if self.player:hasSkill("polu") and card:isKindOf("Weapon") then v = 0 end
        if self.player:hasSkill("wushen") and card:getSuit() == sgs.Card_Heart then v = v + 0.03 end
        if self.player:hasSkill("luawushen") and card:getSuit() == sgs.Card_Heart then v = v + 0.3 end
        ...
        if self.player:hasSkill("lualianhua") and card:getEffectiveId() == self.player:getMark("lualianhua_id") and self.player:getPhase() == sgs.Player_Play and self.player:getMark("lualianhua_triggering") > 0 and self.player:getMark("lualianhua_used") == 0 then v = v + 0.09 end
        if self.player:hasSkill("tianqiao") and card:getEffectiveId() == self.player:getMark("tianqiao_id") and self.player:getPhase() == sgs.Player_Play and self.player:getMark("tianqiao_triggering") > 0 and self.player:getMark("tianqiao_used") == 0 then v = v + 0.09 end
        if self.slashAvail == 1 then
	...
	if card:hasFlag("&migao") then
		for _, p in sgs.qlist(self.room:findPlayersBySkillName("mogao")) do
			if p:getLevelSkillState("mogao") >= 2 then
				if self:isFriend(p, self.player) then
					v = v + 0.1
				elseif self:isEnemy(p, self.player) then
					v = v - 0.1
	...
	if self.player:hasSkills("yingyi|xuanlin") and card:isBlack() then	--不优先用黑牌
		v = v - 0.01
	end
	
	if self.player:hasSkills("huahuan") and card:getSuit() == sgs.Card_Club then	--不优先用梅花牌
		v = v - 0.01
	end
...
function SmartAI:getDynamicUsePriority(card)
    ...
    if self.player:hasSkill("lianjie") and (self.player:getMark("qiaohui_born") == 0 or self.player:getMark("lianjie_count") < 2) and self.player:getMark("&lianjie+"..getTypeString(card)) > 0 then	--连结
        return self:getUsePriority(card) + 50
    end
	
	local xixue_candraw_suit = {"club", "spade", "diamond", "heart"}
    if self.player:hasSkill("xixue") and card:getSuit() >= 0 and card:getSuit() <= 3 and self.player:getMark("&xixue+"..xixue_candraw_suit[card:getSuit()+1].."_char") > 0 then	--袭穴
        return self:getUsePriority(card) + 50
    end
	
    if self.player:hasSkills("jinzhou") and ((self.player:isWounded() and (card:isKindOf("SilverLion") or card:isKindOf("Ssxuehe"))) or card:isKindOf("Ssxianghu")) then	--先装装备再用能卸载的技能
        return self:getUsePriority(card) + 50
    end
	
    if self.player:hasSkill("yuanhui") and self.player:getMark("yuanhui_black") + self.player:getMark("yuanhui_red") == 1 then	--渊回
		if card:isBlack() and self.player:getMark("yuanhui_black") == 1 then
			return self:getUsePriority(card) + 50
		elseif card:isRed() and self.player:getMark("yuanhui_red") == 1 then
			return self:getUsePriority(card) + 50
    ...
    if self.player:getMark("&shenban") > 0 then	--神伴
		if card:isKindOf("ExNihilo") or card:isKindOf("Snatch") or card:isKindOf("AOE") or card:isKindOf("AmazingGrace") then
			return self:getUsePriority(card) + 50
		elseif card:isKindOf("IronChain") or card:isKindOf("Fudichouxin") or card:isKindOf("Collateral") then
			return self:getUsePriority(card) - 50
		end
    end
	
    if self.player:hasSkill("qilv") then	--奇虑
		if (card:isKindOf("Weapon") and not self.player:getWeapon()) or (card:isKindOf("OffensiveHorse") and not self.player:getOffensiveHorse()) then
			return self:getUsePriority(card) + 10
		elseif card:isKindOf("Analeptic") then
			return self:getUsePriority(card) + 13
		end
    end
	
    if self.player:hasSkill("longxi") and self.player:getMark("longxi_used") == 0 then	--龙息，第一张优先用不指定自己的牌
		if card:isKindOf("IronChain") or card:isKindOf("AOE") or card:isKindOf("AmazingGrace") or card:isKindOf("GodSalvation") then
			return self:getUsePriority(card) + 11
		elseif card:isKindOf("Slash") or (card:isKindOf("TrickCard") and not card:isKindOf("ExNihilo")) then
			return self:getUsePriority(card) + 10
		end
    end
	
    if self.player:hasSkill("jvhe") then	--居合，将武器优先级调到杀之后。杀优先级为2.5~2.6，武器优先级为2.63~2.69s
		if card:isKindOf("Weapon") then
			return self:getUsePriority(card) - 0.2
		end
    end
	
    if self.player:hasSkill("chenmeng") then	--沉梦，技能发动优先级为4.2，将过拆延后，乐兵桃提前
		if card:isKindOf("Dismantlement") then	--过拆降低到4.1
			return self:getUsePriority(card) - 0.3
		elseif card:isKindOf("Peach") or card:isKindOf("Indulgence") or card:isKindOf("SupplyShortage") then
			return self:getUsePriority(card) + 4.2
		end
    end
	
    if self.player:getMark("&motiao_using") > 0 and (card:isKindOf("Peach") or card:isKindOf("Analeptic") or card:isKindOf("EquipCard") or card:isKindOf("ExNihilo") or card:isKindOf("AmazingGrace") or card:isKindOf("GodSalvation") or card:isKindOf("Fudichouxin")) then	--抹挑，能自己用的优先级调高
        return self:getUsePriority(card) + 50
    end
	
    if self.player:hasSkill("wuji_buding") and card:getSuit() >= 0 and card:getSuit() <= 3 and self.player:getMark("wuji_buding_suit_"..card:getSuitString()) == 0 then	--无忌
        return self:getUsePriority(card) + 10
    end
	
    if self.player:hasSkill("bujuan") and card:isKindOf("OffensiveHorse") then	--不倦，优先用进攻马，优先级由2.69上调至8.09
        return self:getUsePriority(card) + 5.4
    end
	
	if self.player:hasSkill("luashengshi") and self.player:getTag("luashengshiType"):toString() ~= "" and getTypeString(card) ~= self.player:getTag("luashengshiType"):toString() then
		return self:getUsePriority(card) + 50
	end
	if self.player:hasSkill("lianpian") and self.player:getMark("lianpian_triggered") < 3 then
		if (card:isKindOf("Peach") and self.player:isWounded()) or card:isKindOf("Analeptic") or card:isKindOf("EquipCard") or card:isKindOf("ExNihilo") or card:isKindOf("AmazingGrace") or card:isKindOf("GodSalvation") then
			if self.player:getMark("lianpian_used") > 0 then
				return self:getUsePriority(card) + 50
			else
				return self:getUsePriority(card) * 0.1
			end
		end
	end
	if self.player:hasSkill("luaaotu") and self.player:getTag("luaaotuType"):toString() ~= "" then
		if self.player:getTag("luaaotuType"):toString() == "basic" then
			if (card:isKindOf("Snatch") or card:isKindOf("SupplyShortage") or card:isKindOf("Slash")) then
				return self:getUsePriority(card) + 10
			end
		elseif self.player:getTag("luaaotuType"):toString() == "trick" then
			if (card:isKindOf("Peach") and self.player:isWounded()) or card:isKindOf("Analeptic") or card:isKindOf("EquipCard") or card:isKindOf("ExNihilo") or card:isKindOf("AmazingGrace") or card:isKindOf("GodSalvation") then
				return self:getUsePriority(card) * 0.1
			end
		elseif self.player:getTag("luaaotuType"):toString() == "equip" then
			if card:isKindOf("Slash") then
				if card:hasFlag("drank") then
					return self:getUsePriority(card) + 50
				else
					return self:getUsePriority(card) + 25
				end
			end
			if card:isKindOf("TrickCard") then
				if card:isKindOf("Duel") then
					return self:getUsePriority(card) + 40
				elseif card:isKindOf("SavageAssault") or card:isKindOf("ArcheryAttack") then
					return self:getUsePriority(card) + 30
				elseif card:isKindOf("Collateral") then
					return self:getUsePriority(card) + 17
				else
					return self:getUsePriority(card) + 15
				end
			end
		end
	end
	if self.player:hasSkill("luaqinguo") and not self.player:hasSkills("luazhiyan+luapojun+luajiang") then
		if card:isKindOf("Analeptic") and (self:getOverflow() == 3 or self:getOverflow() == 1) then
			return self:getUsePriority(card) * 0.1
		end
	end
	if self.player:hasSkill("luahuoji") and self.player:getMark("luahuoji") == 0 then
		if card:getSkillName() == "luahuoji" then
			return 8.95
		end
	end
	if self.player:hasSkills("duanliang|ol_duanliang") and self.player:hasSkills("jiezi") then
		if card:getSkillName() == "duanliang" or card:getSkillName() == "ol_duanliang" then
			return 6
		end
	end
	if self.player:hasSkills("guose|nosguose|luaqingcheng") and self.player:hasSkills("huoshui") then
		if card:getSkillName() == "guose" or card:getSkillName() == "nosguose" or card:getSkillName() == "luaqingcheng" then
			return 6
		end
	end
    ...
    if card:isKindOf("Peach") and self.player:hasSkills("kuanggu|kofkuanggu|ol_kuanggu") then return 1.01 end
    if card:isKindOf("Snatch") and self.player:hasSkills("luatanbei") then return 2.61 end
    ...
    if self.player:hasSkill("danshou") and not self.player:hasSkills("jueqing|lualiezhi")
    ...
    elseif card:isKindOf("ArcheryAttack") and self.player:hasSkills("luanji|mobileluanji") then
        value = value + 6.0
...
function SmartAI:cardNeed(card)
    ...
    if card:isKindOf("Crossbow") and self.player:hasSkills("luoshen|yongsi|kurou|keji|wusheng|wushen|luawushen|chixin|fenxin_S|jichi") then return 20 end
    if card:isKindOf("Axe") and self.player:hasSkills("luoyi|lualuoyi|jiushi|luajiushi|jiuchi|pojun|luapojun|bianpin") then return 15 end
...
function SmartAI:isPoisonousCard(card, player, from)	--新函数，用于判断某角色是否不需要获得某牌
	if not card then return false end
	player = player or self.player
	from = from or self.player
	
	if player:hasSkills("guatian") and card:isKindOf("Slash") and getCardsNum("Slash", player, from, true) == 0 then	--瓜田，不要塞杀
		return true
	end
	return false
end

function SmartAI:needCard(to, card)		--新函数，返回某角色是否需要某张牌（根据cardneed）
	if to and card then
		for _, askill in sgs.qlist(to:getVisibleSkillList(true)) do
			local callback = sgs.ai_cardneed[askill:objectName()]
			if type(callback) == "function" and callback(to, card, self) then return not self:isPoisonousCard(card, to) end
		end
	end
	return false
end

function SmartAI:needDamage(from, to)		--新函数，返回某角色是否需要造成伤害
	if from then
		for _, askill in sgs.qlist(from:getVisibleSkillList(true)) do
...
function SmartAI:sortIdsByValue(ids, type_name, inverse)	--将id列表按照特定规则排序，如："keep" "use"
    local compare_func = function(a, b)
		local ca = sgs.Sanguosha:getCard(a)
		local cb = sgs.Sanguosha:getCard(b)
		
        local value1 = 0
        local value2 = 0
		
		if type_name == "use" then
			value1 = self:getUseValue(ca)
			value2 = self:getUseValue(cb)
		elseif type_name == "keep" then
			value1 = self:getKeepValue(ca)
			value2 = self:getKeepValue(cb)
		elseif type_name == "priority" then
			value1 = self:getUsePriority(ca)
			value2 = self:getUsePriority(cb)
		elseif type_name == "dynamic_priority" then
			value1 = self:getDynamicUsePriority(ca)
			value2 = self:getDynamicUsePriority(cb)
		elseif type_name == "card_need" then
			value1 = self:cardNeed(ca)
			value2 = self:cardNeed(cb)
		end
        ...
        if value1 ~= value2 then
            ...
            if not inverse then return ca:getNumber() > cb:getNumber() end
            return ca:getNumber() < cb:getNumber()
        end
    end

    table.sort(ids, compare_func)
end
...
function sgs.isRolePredictable(classical)
    ...
    if (not mode:find("0") and not isMini) or mode =="02p" or mode =="02_1v1" or mode =="04_1v3"
        or mode =="08_defense" or mode =="04_boss" or mode == "06_3v3" or mode == "06_XMode" or mode == "04_tt" or mode == "04_if"	--明身份模式
...
sgs.ai_card_intention.general = function(from, to, level)
    ...
    if (from and from:isLord()) or level == 0 then return end
...
function SmartAI:isFriend(other, another)
	if self.room:getMode() == "06_boss" or self.room:getMode() == "05_boss" then
		return other:isYourFriend(another)
	end
...
function SmartAI:isEnemy(other, another)
	if self.room:getMode() == "06_boss" then
		if another then
			if (other:isLord() and another:getGeneralName() ~= "zhuyin") or (another:isLord() and other:getGeneralName() ~= "zhuyin") then
				return true 
			end
		else
			if (self.player:isLord() and other:getGeneralName() ~= "zhuyin") or (other:isLord() and self.player:getGeneralName() ~= "zhuyin") then
				return true 
			end
		end
	end
...
function getTrickIntention(trick_class, target)
    ...
    elseif type(intention == "function") then
        ...
        elseif trick_class == "Drowning" then
            if target and target:getArmor() and target:hasSkills("yizhong|bazhen|xiangrui|bingshen") then return 0 else return 60 end
        end
    end
...
function SmartAI:filterEvent(event, player, data)
    ...
    if event == sgs.TargetConfirmed then
        ...
        if sgs.turncount <= 1 and lord and who and from and from:objectName() == player:objectName() and sgs.evaluatePlayerRole(from) == "neutral" then
                if (card:isKindOf("FireAttack")
                    or ((card:isKindOf("Dismantlement") or card:isKindOf("Snatch"))
                        and not self:needToThrowArmor(who) and not (who:hasSkills("tuntian|luatuntian_jie|luatuntian_jieh") and who:hasSkills("zaoxian|luafengchan"))
                    ...
                    or (card:isKindOf("YinlingCard") and not self:needToThrowArmor(who) and not (who:hasSkills("tuntian|luatuntian_jie|luatuntian_jieh") and who:hasSkills("zaoxian|luafengchan"))
                        and not (who:getCards("e"):length() > 0 and self:hasSkills(sgs.lose_equip_skill, who))
                        and not (self:needKongcheng(who) and who:getHandcardNum() == 1))
                    or (card:isKindOf("Slash") and not (self:getDamagedEffects(who, player, true) or self:needToLoseHp(who, player, true, true))
                        and not ((who:hasSkills("leiji|nosleiji|olleiji|xiaoan") or (who:hasSkills("tuntian|luatuntian_jie|luatuntian_jieh") and who:hasSkills("zaoxian|luafengchan"))) and getCardsNum("Jink", who, from) > 0))
        ...
        if card:isKindOf("AOE") and self.player:objectName() == player:objectName() then
            for _, t in sgs.qlist(struct.to) do
                ...
                if t:hasSkill("xiaoan") and not t:isKongcheng() then sgs.ai_AOE_data = data break end
    ...
    elseif event == sgs.PreDamageDone then
        ...
        if clear and damage.to:isChained() then
            for _, p in sgs.qlist(self.room:getOtherPlayers(damage.to)) do
                if p:isChained() and damage.nature ~= sgs.DamageStruct_Normal and damage.nature ~= sgs.DamageStruct_Light then
                    clear = false
                    break
                end
            end
        ...
        if not clear then
            if damage.nature ~= sgs.DamageStruct_Normal and damage.nature ~= sgs.DamageStruct_Light and not damage.chain then
                ...
                for _, p in sgs.qlist(self.room:getAlivePlayers()) do
                    ...
                    elseif p:objectName() ~= damage.to:objectName() and p:isChained() and self:damageIsEffective(p, damage.nature, damage.from) then
                        if damage.nature == sgs.DamageStruct_Fire then
							...
							if p:hasSkill("bingshen") --[[and damage.card and damage.card:isKindOf("Slash")]] then
								added = added + 1 or added
							end
                            sgs.ai_NeedPeach[p:objectName()] = damage.damage + 1 + added - p:getHp()
                        elseif damage.nature == sgs.DamageStruct_Thunder then
                            added = p:hasArmorEffect("toujing") and added + 1 or added
    ...
    elseif event == sgs.CardsMoveOneTime then
        ...
        for i = 0, move.card_ids:length()-1 do
            ...
            if (reason.m_skillName == "qiaobian" or reason.m_skillName == "fahun") and from and to and self.room:getCurrent():objectName() == player:objectName() then
            ...
            -- 张角用
            if player:hasFlag("AI_Playing") and player:hasSkills("leiji|nosleiji|olleiji|xiaoan") and player:getPhase() == sgs.Player_Discard and isCard("Jink", card, player)
            ...
            if player:hasFlag("AI_Playing") and sgs.turncount <= 3 and player:getPhase() == sgs.Player_Discard
                ...
                if isCard("Slash", card, player) and not player:hasFlag("hasUsedSlash") or player:hasFlag("JiangchiInvoke") then
                    for _, target in sgs.qlist(self.room:getOtherPlayers(player)) do
                        ...
                        for _, askill in sgs.qlist(target:getVisibleSkillList(true)) do
                            ...
                            if filter and type(filter) == "function" and not (s_name == "tiandu" or s_name == "luatiandu" or s_name == "hujia" or s_name == "huilei" or s_name == "weidi") then
								...
								elseif s_name == "luaduli" and player:getHp() >= target:getHp() then
									local basic_num = 0
                                    for _, c_id in sgs.qlist(move.card_ids) do
                                        local c = sgs.Sanguosha:getCard(c_id)
                                        if c:isKindOf("BasicCard") then
                                    ...
                                    if basic_num < 2 then has_slash_prohibit_skill = true break end
								--[[elseif s_name == "mingxian" then
									local slash_num = 0
                                    for _, c_id in sgs.qlist(move.card_ids) do
                                        local c = sgs.Sanguosha:getCard(c_id)
                                        if c:isKindOf("Slash") then
                                            slash_num = slash_num + 1
											if c:isBlack() and not c:isKindOf("FireSlash") and not c:isKindOf("ThunderSlash") then
												break
											end
                                        end
                                    end
                                    if slash_num < 2 then has_slash_prohibit_skill = true break end]]
                                else
                ...
                if not (zhanghe and lord and self:playerGetRound(zhanghe) <= self:playerGetRound(lord) and self:isFriend(zhanghe, lord)) then
                    ...
                    if isCard("SupplyShortage", card, player) and lord and not lord:hasSkill("qiaobian") then
                        for _, target in sgs.qlist(self.room:getOtherPlayers(player)) do
                            if (player:distanceTo(target) <= (player:hasSkill("duanliang") and 2 or 1) or (player:hasSkill("ol_duanliang") and player:getHandcardNum() <= target:getHandcardNum())) and
...
function SmartAI:askForNullification(trick, from, to, positive)
    ...
    if from and not from:hasSkills("jueqing|lualiezhi") and not to:hasSkills("lualiezhi") then
    ...
    if positive then
        ...
        if from and self:isEnemy(from) and (sgs.evaluatePlayerRole(from) ~= "neutral" or sgs.isRolePredictable()) then
            ...
            if self:isFriend(to) then
                if trick:isKindOf("Dismantlement") then
					...
					if trick:isKindOf("Fudichouxin") then	--釜底抽薪（对友）
						if to:getArmor() and not self:needToThrowArmor(to) and self:isWeak(to) then
							return null_card
						elseif self:willSkipPlayPhase(to) and self:getOverflow(to) + to:getEquips():length() > 1 then
							return null_card
						end
					end
					if trick:isKindOf("BefriendAttacking") then	--远交近攻（对友）
						local cp = to:getTag("spouse"):toPlayer()
						if cp and cp:isAlive() and self:isWeak(cp) and self:damageIsEffective(cp, sgs.DamageStruct_Normal, from) then
							return null_card
						end
					end
                end
            elseif self:isEnemy(to) then
				...
				if trick:isKindOf("Fudichouxin") and to:getArmor() and self:needToThrowArmor(to) then	--釜底抽薪（对敌）
					return null_card
				end
				if trick:isKindOf("BefriendAttacking") then		--远交近攻（对敌）
					local cp = to:getTag("spouse"):toPlayer()
					if not cp or cp:isDead() or not self:damageIsEffective(cp, sgs.DamageStruct_Normal, from) or self:needToLoseHp(cp) or cp:hasSkills(sgs.masochism_skill) then
						return null_card
					end
				end
            end
        ...
        if self:isFriend(to) then
            if not ((to:hasSkills("guanxing|mengjian") and global_room:alivePlayerCount() > 4) or to:hasSkills("yahe")) then
                ...
                if trick:isKindOf("Indulgence") and not to:isSkipped(sgs.Player_Play) then
                    ...
                    if to:hasSkills("qiaobian|fahun") and not to:isKongcheng() then return nil end
                ...
                if trick:isKindOf("SupplyShortage") and not to:isSkipped(sgs.Player_Draw) then
                    if self:hasSkills("guidao|tiandu|luatiandu",to) then return nil end
            ...
            if self:isFriend(NP) then
                ...
                if peach_num == 0 and not self:willSkipPlayPhase(NP) then
                    if exnihilo_num > 0 then
                        if NP:hasSkills("nosjizhi|jizhi|nosrende|zhiheng|zhuoshi|qilv|zhanshu|qisheng")
                        ...
                        if not from:hasSkills("jueqing|lualiezhi") and not to:hasSkills("lualiezhi") and fa_card then
...
function SmartAI:askForCardChosen(who, flags, reason, method)
    ...
    if self:isFriend(who) then
        if flags:match("j") and not who:containsTrick("YanxiaoCard") and not (who:hasSkills("qiaobian|fahun") and who:getHandcardNum() > 0) then
        ...
        if flags:match("h") and (not isDiscard or self.player:canDiscard(who, "h")) then
            if who:hasSkills("jijiu|qingnang|qiaobian|jieyin|beige|buyi|manjuan|yizan|quanneng|fahun|wenyu")
...
function sgs.ai_skill_cardask.nullfilter(self, data, pattern, target)
    ...
    if target and target:hasSkills("jueqing|lualiezhi") and self:needToLoseHp() then return "." end
    if target and target:hasSkills("jueqing|lualiezhi") then return end
    ...
    if effect and target and (target:hasWeapon("ice_sword") or (self.is_ice_slash and not self.ignore_ice)) and self.player:getCards("he"):length() > 1 then return end
    ...
    if self:needBear(self.player, true) then return "." end
...
function SmartAI:askForCard(pattern, prompt, data)
	...
	--local ice_slash_jink = false
	--if parsedPrompt[1] == "slash-jink" and type(data) == "userdata" then	--冰杀询问闪特殊处理
	--	local effect = data:toSlashEffect()
	--	local slash = effect.slash
	--	if slash and slash:isKindOf("IceSlash") then
	--		ice_slash_jink = true
	--		self.is_ice_slash = true
	--	end
	--end
    local callback = sgs.ai_skill_cardask[parsedPrompt[1]]
	--if ice_slash_jink then
	--	self.is_ice_slash = false
	--end
    if type(callback) == "function" then
...
function sgs.ai_cardneed.damagetricks(to, card, self)
    if not self:willSkipPlayPhase(to) then
        return card:isKindOf("AOE") or card:isKindOf("Duel") or card:isKindOf("FireAttack")
    end
end

function sgs.ai_cardneed.damagecards(to, card, self)
    if not self:willSkipPlayPhase(to) then
        return card:isKindOf("AOE") or card:isKindOf("Duel") or card:isKindOf("FireAttack") or card:isKindOf("Slash")
    end
end
...
function SmartAI:getDamageAdjustment(from, to, card, dmg, nature)		--新函数，用于拓展hasHeavySlashDamage仅受限于杀的问题，该函数返回处理后的伤害数
	...
	--if not self:isFriend(from, to) then			--对非友方的处理部分（通常是一些次数免疫技能，敌方有必要打，但友方不能通过它传导伤害。对友方免疫的部分见 damageIsEffective_）
    	if to:hasSkills("yinqi") and to:isKongcheng() then	--阴憩对敌人视为能打1伤害，避免ai酒杀
			return 1
		end
    --end
    if nature == sgs.DamageStruct_Fire and (to:hasArmorEffect("vine") and not IgnoreArmor(from, to)) or to:hasSkills("bossmanjia") then	--火伤加成：藤甲/青衿
		dmg = dmg + 1
    end
    if nature == sgs.DamageStruct_Fire and to:hasSkills("bingshen") then	--火伤加成：冰身
		dmg = dmg + 1
    end
    if nature == sgs.DamageStruct_Thunder and (to:hasArmorEffect("toujing") and not IgnoreArmor(from, to)) then	--雷伤加成：透镜
		dmg = dmg + 1
    end
	
    if from:hasSkill("bianpin") and from:getMark("bianpin_B") > 0 then dmg = dmg + 1 end	--变频加伤
    if from:getMark("jili_magi_inc1") > 0 then dmg = dmg + 1 end	--极理加伤
    if from:getMark("jili_magi_dec1") > 0 then dmg = dmg - 1 end	--极理减伤
    if from:hasSkill("youlan") and card and card:isKindOf("TrickCard") then dmg = dmg + 1 end	--幽蓝加伤
    if to:hasSkill("youlan") and card and card:isKindOf("TrickCard") then dmg = dmg - 1 end	--幽蓝减伤
    if from:hasSkill("maodun") then	--矛盾加伤（来源）
		local is_max_hp = true
		for _,p in sgs.qlist(self.room:getOtherPlayers(from)) do
			if p:getHp() >= from:getHp() then
				is_max_hp = false
				break
			end
		end
    ...
    if to:hasSkill("maodun") then	--矛盾加伤（目标）
		local is_max_hp = true
...
function SmartAI:hasHeavySlashDamage(from, slash, to, getValue)
    ...
    if not from:hasSkills("jueqing|lualiezhi") and not to:hasSkills("lualiezhi") and (to:hasArmorEffect("silver_lion") and not IgnoreArmor(from, to)) then
	...
	if from:hasWeapon("IceSword") and not fireSlash and not thunderSlash and not iceSlash then	--寒冰剑转化普通杀
		iceSlash = true
	end
	...
	if (from:hasSkill("luawushen") and slash and slash:isRed()) then
		dmg = dmg + 1
	end
    if from:hasFlag("luoyi") then dmg = dmg + 1 end
    if from:hasFlag("neoluoyi") then dmg = dmg + 1 end
    if from:hasFlag("lualuoyi") then dmg = dmg + 1 end
    ...
    if not from:hasSkills("jueqing|lualiezhi") and not (to and to:hasSkills("lualiezhi")) then
		...
		elseif thunderSlash then
			nature = sgs.DamageStruct_Thunder
		elseif iceSlash then
			nature = sgs.DamageStruct_Ice
		end
		dmg = self:getDamageAdjustment(from, to, slash, dmg, nature)
	
        --if to:hasArmorEffect("vine") and not IgnoreArmor(from, to) and fireSlash then dmg = dmg + 1 end			--新函数中已处理
        --if to:hasArmorEffect("toujing") and not IgnoreArmor(from, to) and thunderSlash then dmg = dmg + 1 end		--新函数中已处理
...
function SmartAI:needKongcheng(player, keep)
	...
	if player:hasSkills("yongning") and player:getHp() ~= 1 and player:getPhase() ~= sgs.Player_NotActive then	--永宁，若非1血则回合内打空
		return true
	end
	
	if player:hasSkills("heli") --[[and player:getMark("heli") == 0]] then	--鹤唳，未觉醒则打空
		return true
	end
	
	if player:hasSkill("milianvs") then	--秘恋，有二级秘恋且有秘恋敌人则打空
		...
		for _,p in sgs.qlist(self.room:getAlivePlayers()) do
			...
			if p:hasSkill("milianvs") and self:isEnemy(p, player) then
				is_enemy = true
			end
		end
		if is_level_up and is_enemy then
			return true
		end
	end
	
	if player:hasSkills("yuechao") and player:getMark("yuechao_used") == 0 then	--月潮能用
		return true
	end
	
	if player:hasSkills("yinqi") and self:isWeak(player, player == self.player) then	--阴憩且濒死则需要空城
		return true
	end
...
function SmartAI:getLeastHandcardNum(player)
    player = player or self.player
    local least = 0
    if player:hasSkills("lianying|noslianying|lualianying|guatian|hengxin|liucai") and least < 1 then least = 1 end
...
function SmartAI:getCardNeedPlayer(cards, include_self)
    ...
    for _, player in ipairs(friends) do
        if player:hasSkill("jieming") or player:hasSkills("jijiu|yizan|quanneng") then
    ...
    if specialnum > 1 and #cardtogivespecial == 0 and self.player:hasSkill("nosrende") and self.player:getPhase() == sgs.Player_Play then
        ...
        local erzhao = self.room:findPlayerBySkillName("yizan")
        local mien = self.room:findPlayerBySkillName("quanneng")
        ...
        if self.player:getHandcardNum() > #cardtogivespecial and redcardnum > 0 then
            for _, hcard in ipairs(cardtogivespecial) do
                ...
                if hcard:isKindOf("BasicCard") then return hcard, mien end
                if hcard:isKindOf("BasicCard") then return hcard, erzhao end
                return hcard, xunyu
            end
        end
    end
    ...
    if (self.player:hasSkill("rende") and not self.player:hasUsed("RendeCard") and self.player:isWounded() and self.player:getMark("rende") < 2) then
        ...
        if ((self.player:getHandcardNum() == 2 and self.player:getMark("rende") == 0) or
            ...
            for _, enemy in ipairs(self.enemies) do
                ...
                (enemy:canSlash(self.player) or enemy:hasSkill("shensu") or enemy:hasSkills("wushen|luawushen") or enemy:hasSkill("jiangchi")) then return end
    ...
    for _, friend in ipairs(friends) do
        if friend:getHp()<=2 and friend:faceUp() then
            for _, hcard in ipairs(cards) do
                if (hcard:isKindOf("Armor") and not friend:getArmor() and not self:hasSkills("yizhong|bazhen|xiangrui|bingshen",friend))
    ...
    for _, friend in ipairs(friends) do
        if friend:hasSkills("jijiu|jieyin|yizan|quanneng") and friend:getHandcardNum() < 4 then
            for _, hcard in ipairs(cards) do
                if (hcard:isRed() and friend:hasSkill("jijiu")) or (hcard:isKindOf("BasicCard") and friend:hasSkills("yizan|quanneng")) or friend:hasSkill("jieyin") then
                    return hcard, friend
                end
            end
        end
    ...
    --Crossbow
    for _, friend in ipairs(friends) do
        if friend:hasSkills("longdan|wusheng|keji|chixin|fenxin_S|jichi") and not self:hasCrossbowEffect(friend) and friend:getHandcardNum() >= 2 then
            for _, hcard in ipairs(cards) do
    ...
    for _, shit in ipairs(cardtogive) do
        if shit:isKindOf("Shit") then
            for _,friend in ipairs(friends) do
                ...
                elseif shit:getSuit() == sgs.Card_Spade or friend:hasSkills("jueqing|lualiezhi") then
...
function SmartAI:askForPindian(requestor, reason)
    local passive = {"mizhao", "lieren", "lualieren" , "libeng", "bidui", "zhengyu"}
    ...
    if self:isFriend(requestor) then
		...
		if self.player:hasSkills("luabixian") and maxcard:getNumber() <= self:getMaxCard(requestor):getNumber() then
			for _, card in ipairs(cards) do
				if card:isKindOf("Slash") then
					return card
				end
...
function SmartAI:ableToSave(saver, dying)
    ...
    if current and current:getPhase() ~= sgs.Player_NotActive and current:hasSkill("wansha")
        ...
        and not saver:hasSkills("jiuzhu|chunlao|nosjiefan|renxin|luazuijiao|luazhafu|yanling|shisui") then
        return false
    end
    local peach = sgs.Sanguosha:cloneCard("peach", sgs.Card_NoSuitRed, 0)
    if saver:isCardLimited(peach, sgs.Card_MethodUse, true) and not saver:hasSkills("jiuzhu|chunlao|nosjiefan|renxin|luazuijiao|luazhafu|yanling|shisui|fuyin_shayue") then
		peach:deleteLater()
		return false
	end
	peach:deleteLater()
...
function SmartAI:willUsePeachTo(dying)
    ...
    if self.player:objectName() == dying:objectName() and not self:needDeath(dying) then
        ...
        if not self.player:isLocked(analeptic) and self:getCardId("Analeptic", self.player, nil, true) then
			analeptic:deleteLater()
			return self:getCardId("Analeptic", self.player, nil, true)
		end
		analeptic:deleteLater()
        if self:getCardId("Peach", self.player, nil, true) then return self:getCardId("Peach", self.player, nil, true) end
    end
--[[ 
...
--]]
    ...
    if self.room:getMode() ~= "04_if" and (sgs.ai_role[dying:objectName()] == "renegade" and dying:objectName() ~= self.player:objectName()) then
    ...
    if self:isFriend(dying) then
        ...
        local lord = getLord(self.player)
        if not sgs.GetConfig("EnableHegemony", false) and self.room:getMode() ~= "04_if" and self.player:objectName() ~= dying:objectName() and not dying:isLord() and
        ...
        local buqu = dying:getPile("trauma")
        local weaklord = 0
        ...
        if self.player:objectName() ~= dying:objectName() and dying:hasSkills("jiushi|luajiushi") and dying:faceUp() and dying:getHp()== 0 then
            return "."
        end

        if (self.player:objectName() == dying:objectName()) then
            card_str = self:getCardId("Analeptic", self.player, nil, true)
            if not card_str then
                card_str = self:getCardId("Peach", self.player, nil, true)
            end
       ...
       -- 鞭尸...
        ...
        if not dying:hasSkills(sgs.masochism_skill) and not hasBuquEffect(dying)
            ...
            and not (dying:hasSkill("shengguangN") and dying:getMark("@halo") > 0)
...
function SmartAI:activate(use)
    ...
    for _, card in ipairs(self.toUse) do
        if not self.player:isCardLimited(card, card:getHandlingMethod())
            or (card:canRecast() and not self.player:isCardLimited(card, sgs.Card_MethodRecast) and not (self.player:hasSkills("lualianhua|tianqiao") and card:getId() == self.room:getDrawPile():first())) then	--不允许AI用炼化（天巧）重铸
            local type = card:getTypeId()
...
function SmartAI:willUse(player, card, ignoreDistance, disWeapon, play)	--yun
	if not card then self.room:writeToConsole(debug.traceback()) return false end
	from = player or self.player
	if play == nil then
		play = (player:getPhase() == sgs.Player_Play)
	end
	if play then
		if card:isKindOf("BasicCard") then
			if card:isKindOf("Peach") and from:getLostHp() <= 0 then return false end
		...
		if card:isKindOf("Slash") then
			for _,to in ipairs(self.enemies) do
				local far = false
				if from:hasSkill("ol_liegong") or from:hasFlag("TianyiSuccess") or from:hasFlag("JiangchiInvoke") or from:hasFlag("InfinityAttackRange") then far = true end
				if disWeapon then
...
function SmartAI:getOverflow(player, getMaxCards)
	...
	if player:hasSkill("luaxingzhao") then
		local injured_count = 0
		for _, p in sgs.qlist(self.room:getAlivePlayers()) do
			if p:isWounded() then
				injured_count = injured_count + 1
	...
	if MaxCards == 0 then
		...
		local MaxCards_changed = false
		
		if player:hasSkill("luabixian") or (player:hasSkill("luajiangchi") and player:hasFlag("luajiangchi_keep")) then
			for _,cd in sgs.qlist(player:getHandcards()) do
				if cd:isKindOf("Slash") then
					MaxCards_changed = true
					MaxCards = MaxCards + 1
				end
			end
		end
		if player:hasSkill("lualuoying") then
			for _,cd in sgs.qlist(player:getHandcards()) do
				if cd:getSuit() == sgs.Card_Club and not cd:isKindOf("BasicCard") then
					MaxCards_changed = true
					MaxCards = MaxCards + 1
				end
			end
		end
		if player:hasSkill("luayoudi") then		--诱敌可以多“存”一张
			MaxCards_changed = true
			MaxCards = MaxCards + 1
		end
		...
		for _, card in sgs.list(player:getHandcards()) do
			if player:getMark("luazongshih_"..card:getId()) > 0 then
				count = count + 1
			end
		end
		if count > 0 then
			MaxCards_changed = true
			MaxCards = MaxCards + count
		end
		
		if player:hasSkill("shixi") and player:getPhase() ~= sgs.Player_Discard then		--势惜，弃牌阶段外特殊手牌上限处理（弃牌阶段无需处理）
			local kingdoms = {}
			for _,p in sgs.qlist(self.room:getAlivePlayers()) do
			...
			if X > 1 then
				MaxCards_changed = true
				MaxCards = MaxCards + player:getLostHp() - player:getHp()
			end
		end
		
		if player:hasSkill("xuanlin") and player:getPhase() ~= sgs.Player_Discard then		--玄鳞，黑牌不计入手牌数，弃牌阶段外特殊手牌上限处理
			for _,cd in sgs.qlist(player:getHandcards()) do
				if cd:isBlack() then
					MaxCards_changed = true
					MaxCards = MaxCards + 1
		...
		if player:hasSkill("huahuan") and player:getPhase() ~= sgs.Player_Discard then		--花环，梅花牌不计入手牌数，弃牌阶段外特殊手牌上限处理
			for _,cd in sgs.qlist(player:getHandcards()) do
		...
		--if MaxCards_changed then
		--	MaxCards = MaxCards + player:getMaxCards()
		--end
	end
    ...
    --end
    if getMaxCards then return MaxCards end
	MaxCards = self:getBestKeepHandcardNum(player)

    return player:getHandcardNum() - MaxCards
end

function SmartAI:isWeak(player, self_view)	--第二个参数表示是否为自己的视角
    player = player or self.player
    local hcard = player:getHandcardNum()
	local save_card = 0
	if self_view then
		save_card = self:getCardsNum("Peach") + self:getCardsNum("Analeptic")
	end
    ...
    if player:hasSkill("shenghua") and player:getMark("shenghua") == 0 and player:getHp() > 1 then return false end
    if (player:getHp() <= 2 and (hcard <= 2 or save_card > 1)) or player:getHp() <= 1 then return true end
    return false
end

function SmartAI:useCardByClassName(card, use)
	if not self:needBearCard(card) then
...
function SmartAI:canRetrial(player, to_retrial, reason)
    ...
    if player:hasSkill("luadongxiao") and player:getHandcardNum() > 0 then return true end
    if player:hasSkill("tonggan") and not player:getPile("zhanshu_pile"):isEmpty() then return true end	--通感
    if player:hasSkill("weiliu") then	--维留
...
function SmartAI:needRetrial(judge)
    ...
    local isEnemy = self:isEnemy(who)
    local isGood = judge:isGood()
	if who:hasSkill("xuxiang") and who:isChained() then
		isGood = not isGood
	end
    if reason == "lightning" then
        ...
        if lord and (who:isLord() or (who:isChained() and lord:isChained())) and self:objectiveLevel(lord) <= 3 then
            ...
            return self:damageIsEffective(lord, sgs.DamageStruct_Thunder) and not isGood
        end
        ...
        if isFriend then
            ...
            if who:isChained() and not self:isGoodChainTarget(who, self.player, sgs.DamageStruct_Thunder, 3) then return isGood end
        end
    end
    ...
    if reason == "supply_shortage" then
        if isFriend then
            if self:hasSkills("guidao|tiandu|luatiandu",who) then return false end
            return not isGood
        else
            return isGood
        end
    ...
    if reason == "luoshen" then
        if isFriend then
            ...
            if self:hasCrossbowEffect(who) or getKnownCard(who, self.player, "Crossbow", false) > 0 then return not isGood end
    ...
    if reason == "tuntian" or reason == "luatuntian_jie" or reason == "luatuntian_jieh" then
        if not who:hasSkill("zaoxian|jixi|ziliang|luafengchan") then return false end
    end

    if reason == "beige" then return true end

	if reason == "luatiandu" then
		if who:getHp() == 1 and self:damageIsEffective(who, sgs.DamageStruct_Normal) then
			return true
		end
		return false
	end

	if reason == "xingyao" then		--对星耀改判处理
		for _, p in sgs.qlist(self.room:getAllPlayers()) do
			if p:hasFlag("xingyao_slash_target") then
				if judge.card:getSuit() == sgs.Card_Heart then
					if p:hasSkills(sgs.masochism_skill) and self:isEnemy(p) then	--对卖血敌人判红桃就改
						return true
					elseif not p:hasSkills(sgs.masochism_skill) and self:isFriend(p) then	--对非卖血友方判红桃就改
						return true
					end
				else
					if p:hasSkills(sgs.masochism_skill) and self:isFriend(p) then
						return true
					elseif not p:hasSkills(sgs.masochism_skill) and self:isEnemy(p) then
						return true
					end
				end
			end
	...
	if reason == "xuelin" then	--对血林改判处理
		if isFriend and judge.card:isRed() and (self:isWeak(who) or who:getCardCount(true) < 2) then
			return true
		elseif isEnemy and not judge.card:isRed() and (self:isWeak(who) or who:getCardCount(true) < 2) then
			return true
		end
	end

	if reason == "fuyin_shayue" and self.player:hasSkills("fuyin_shayue+weiliu") and not self.player:hasFlag("fuyin_shayue_failed") then	--对福音改判处理（仅对沙月自身）
		local success = who:getMark("&fuyin_shayue->"..getNumberChar(judge.card:getNumber())) == 0
		return (isFriend and not success) or (not isFriend and success)
	end
...
function SmartAI:damageIsEffective(to, nature, from, card, damage, real_damage)	--real_damage为真代表根据是否会造成伤害返回值（如受伤后回血等效于无伤的情况将被排除，用于触发技能和触发铁索）
    local damageStruct = {}
    ...
    damageStruct.damage = damage or 1
	real_damage = real_damage or false
    return self:damageIsEffective_(damageStruct, real_damage)
end

function SmartAI:damageIsEffective_(damageStruct, real_damage)
    ...
    local damage_origin = damageStruct.damage or 1	--记录原始伤害（即输入值）
    local damage = damage_origin	--用于后续伤害调整
    local from = damageStruct.from
	local card = damageStruct.card
	real_damage = real_damage or false
    if from:hasSkills("jueqing|lualiezhi") or (to and to:hasSkills("lualiezhi")) then return true end
	...
	if self.room:getLord() and self.room:getLord():getMark("luadanggu_turn_counter") == 1 and self.room:getLord():hasSkill("luadanggu") and (from:getKingdom() == "wu" or to:getKingdom() == "wu") then
		return false
	end
	if self.room:getLord() and self.room:getTag("TurnLengthCount") and self.room:getTag("TurnLengthCount"):toInt() == 1 and self.room:getLord():hasSkill("hehu") and (from:getKingdom() == "niya" or to:getKingdom() == "niya") then	--呵护
		return false
	end
    if to:hasSkills("jgyuhuo|shixin|yuhuo") and nature == sgs.DamageStruct_Fire then	--浴火免伤
        return false
    end
	if from:getMark("qingrong_target_"..to:objectName()) > 0 then	--情容免伤
		return false
	end
	if from:getMark("&motiao_using") > 0 and from:getMark("caused_damage_in_turn") >= 1 then	--抹挑免伤
		return false
	end
	
	if to:hasArmorEffect("neneko") and card and not card:isKindOf("SkillCard") and to:getMark("&neneko_record_"..getTypeString(card)) > 0 then	--猫玩偶
		return false
	end
	if to:getMark("&shisu_protect") > 0 then	--时溯
		return false
	end
    --if to:hasSkill("yuanlin") and nature ~= sgs.DamageStruct_Normal and to:getMark("@shenyuan") + damage <= 13 then
    if to:hasSkill("yuanlin") and nature ~= sgs.DamageStruct_Normal and to:getMark("@shenyuan") < 5 then
        return false
    end
    if to:hasSkills("lixi") and to:getHp() <= 0 and not real_damage then	--离息
	...
	if to:hasSkills("andong") then
		...
		if heart_count > 1 or self:isFriend(to, from) then
			return false
		end
	end
	if to:hasSkill("luacidui") and not from:isKongcheng() and not to:isKongcheng() then
		local numbers = {}
		local repeated = false
		for _,cd in sgs.qlist(to:getHandcards()) do
			if not table.contains(numbers, cd:getNumber()) then
				table.insert(numbers, cd:getNumber())
			else
				repeated = true
				break
			end
    ...
    if from:hasSkill("xiongshou") then
        damage = damage + 1
    end
    if to:hasSkill("xiangde") and from:getWeapon() then
        damage = damage + 1
    end
    ...
    if to:hasSkills("youlan") and card and card:isKindOf("TrickCard") then	--幽蓝减伤
        damage = damage - 1
        if damage == 0 then return false end
    end
    if to:hasSkill("yuce") and not to:isKongcheng() and to:getHp() > 1 and not real_damage then
	...
	if to:hasSkill("lualuohong") and not from:isKongcheng() and to:getHp() > 1 then
		local red_count = 0
		for _,cd in sgs.qlist(from:getHandcards()) do
			if cd:isRed() then
		...
		if self:isFriend(to, from) then
			if 1.0*red_count/from:getHandcardNum() > 0.66 then	--（友方）有三分之二以上红牌
				return false
			end
		else
			if 1.0*red_count/from:getHandcardNum() >= 0.5 then	--有一半以上红牌
				return false
			end
		end
	end
    ...
    if to:hasSkill("xianrou") then		--纤柔，对敌方，没黑桃血不多则视为不能打
    ...
    if to:hasSkills("xuelin|chushou") then		--血林/触手，对敌方，自身吃不了反伤则视为不能打
        if self:isEnemy(to, from) or to:hasSkill("chushou") then
			if self:isWeak(from, true) then
				return false
			end
        end
    end
    if to:hasSkills("yinqi") and to:isKongcheng() then		--阴憩，对友方视为免伤
        if self:isFriend(to, from) then
			return false
        end
    end
    if to:hasSkill("qingling") and to:getHp() > 1 and not real_damage then		--青灵，伤害<1且体力值>1时与猫玩偶相同（但不是真的免伤）
		if card and not card:isKindOf("SkillCard") and to:getMark("&qingling_record_"..getTypeString(card)) > 0 and damage <= 1 then
			return false
		end
    end
    ...
    if to:hasSkill("shibei") and not real_damage then
...
function SmartAI:dontHurt(to, from)	--yun	针对队友
	if from:hasSkill("jueqing") then return true end
	if not from:hasSkill("jueqing") and hasOrangeEffect(to) then return true end
	if to:hasSkills("sizhan|xin_lixun") then return true end
	return false
end

function SmartAI:justDamage(to, from, isSlash, lock)	--yun	针对敌人
	...
	if from:hasSkill("duorui_sec_rev") and from:getMark("has_used_duorui_sec_rev") == 0 then return true end
	if to:getHp() <= 2 and to:hasSkills("chanyuan|jiechanyuan") then return true end
	...
	if from:hasSkills("zhiman|bf_zhiman|lua_zhiman") and not self:doNotDiscard(to, "e") then return true end
	if isSlash then
		if not lock and from:hasSkills("tieji|xingyao") then return true end
		if from:hasSkill("nosqianxi") and from:distanceTo(to) == 1 then
            return true
        end
        if (from:hasWeapon("ice_sword") or (self.is_ice_slash and not self.ignore_ice)) and not self:doNotDiscard(to, "he") then
            return true
        end
        if from:hasSkills("bingxin") and not self:doNotDiscard(to, "he") then
            return true
        end
    end
	return false
...
function SmartAI:canDamage(to, from, slash, nature, card)	--yun
	from = from or self.room:getCurrent()
	to = to or self.player
	nature = nature or sgs.DamageStruct_Normal
...
function SmartAI:getDamagedEffects(to, from, isSlash)
    from = from or self.room:getCurrent()
    to = to or self.player
    if from:hasSkills("jueqing|lualiezhi") then return false end
    if to:hasSkills("lualiezhi") then return false end
    ...
    if isSlash then
        ...
        if (from:hasWeapon("ice_sword") or (self.is_ice_slash and not self.ignore_ice) or (type(isSlash) ~= "boolean" and isSlash:isKindOf("IceSlash"))) and to:getCards("he"):length() > 1 and not self:isFriend(from, to) then
            return false
        end
        if from:hasSkills("bingxin") and to:getCards("he"):length() > 1 and not self:isFriend(from, to) then
            return false
        end
    end
    ...
    if sgs.isGoodHp(to) then
		...
		if from and self:isFriend(from, to) then
			for _, askill in sgs.qlist(from:getVisibleSkillList(true)) do
				local callback = sgs.ai_need_damage[askill:objectName()]
				if type(callback) == "function" and callback(self, from, to) then return true end
			end
...
function SmartAI:shouldNotDamage(who)	--不应造成伤害
	who = who or self.player
	if who:hasSkills("keyu+bingxin") and who:getPhase() ~= sgs.Player_NotActive and who:getPile("keyu_pile"):length() > 1 and who:getMark("bingxin_damaged") == 0 then	--有渴欲和冰心且“欲”>1，回合内不应造成伤害
		return true
	end
end
...
function isCard(class_name, card, player)
    ...
    if not card:isKindOf(class_name) then
        ...
        if player:hasSkills("wushen|luawushen") and card:getSuit() == sgs.Card_Heart and class_name == "Slash" then return true end
        if player:hasSkill("jinjiu") and card:isKindOf("Analeptic") and class_name == "Slash" then return true end
    else
        if player:hasSkills("wushen|luawushen") and card:getSuit() == sgs.Card_Heart and class_name ~= "Slash" then return false end
...
function SmartAI:getMaxCard(player, cards, visible)
    player = player or self.player
    ...
    for _, card in sgs.qlist(cards) do
        ...
        if (player:objectName() == self.player:objectName() and not self:isValuableCard(card)) or self.player:canSeeHandcard(player) or visible or card:hasFlag("visible") or card:hasFlag(flag) then
...
function SmartAI:getCardId(class_name, player, acard, consider_limit)
    player = player or self.player
    local cards
    ...
    for _, card in ipairs(cards) do
		if consider_limit and self:needBearCard(card, self.player, true) then
			continue
		end
    ...
    if #viewArr > 0 or #cardArr > 0 then
		...
		local viewas_first = false
		if viewCard then	--是否优先出技能视为的牌，而非实体牌
			if player:hasSkill("chongzhen") and viewCard and viewCard:getSkillName() == "longdan" then	--龙胆+冲阵 闪
				viewas_first = true
			end
			if player:hasSkill("luajuzhan") and player:getMark("luajuzhan") ~= 1 then	--拒战 闪
				viewas_first = true
			end
			if player:hasSkill("fenxin_S") and viewCard and viewCard:getSkillName() == "fenxin_S" then	--焚心 杀
				viewas_first = true
			end
			if player:hasSkill("jichi") and viewCard and viewCard:getSkillName() == "jichi" and not sgs.Sanguosha:getCard(viewCard:getSubcards():first()):isKindOf("Armor") then	--疾驰 非防具杀
				viewas_first = true
			end
			if player:hasSkill("fengjin") and viewCard and viewCard:getSkillName() == "fengjin" and viewCard:isRed() then	--枫锦 红色无懈
				viewas_first = true
			end
			if player:hasSkill("wenyu") and viewCard and viewCard:getSkillName() == "wenyu" and viewCard:isRed() then	--温御 红色闪/无懈
				viewas_first = true
			end
		end
        return (viewas_first) and (viewas or cardid) or (cardid or viewas)
    end
...
function getCardsNum(class_name, player, from, original, for_others)	--新增第四项original，为真则不统计技能|新功能class_name为"VisibleCard"时返回手牌中的明牌数
    if not player then
    ...
    if not player then
        ...
        for _, card in ipairs(cards) do
            local flag = string.format("%s_%s_%s", "visible", from:objectName(), player:objectName())
            if card:hasFlag("visible") or card:hasFlag(flag) or not forbid and (from:objectName() == player:objectName() or from:canSeeHandcard(player)) then
                shownum = shownum + 1
	...
	if class_name == "VisibleCard" then
		return shownum
	end
	...
	if original then
		...
		if class_name == "Slash" then
			local slashnum
			if player:hasSkills("wusheng|fenxin_S") then
				slashnum = redslash + num + (player:getHandcardNum() - shownum) * 0.69
			elseif player:hasSkills("wushen|luawushen") then
				slashnum = heartslash + num + (player:getHandcardNum() - shownum)*0.5
			elseif player:hasSkills("longhun|lualiewei|luasuisheng") then
				slashnum = diamondcard + num + (player:getHandcardNum() - shownum)*0.5
			elseif player:hasSkill("nosgongqi|jichi") then
			...
			elseif player:hasSkill("yizan") then
				slashnum = basiccard + num + (player:getHandcardNum() - shownum)*0.5
			elseif player:hasSkill("quanneng") and player:getMark("quanneng_used") == 0 and basiccard > 0 then
				slashnum = 1 + num + (player:getHandcardNum() - shownum)*0.5
			else
		...
		elseif class_name == "Jink" then
			...
			elseif player:hasSkills("longdan|wenyu") then
			...
			elseif player:hasSkill("yizan") then
				return num + basiccard + (player:getHandcardNum() - shownum)*0.5
			elseif player:hasSkill("quanneng") and player:getMark("quanneng_used") == 0 and basiccard > 0 then
				return num + 1 + (player:getHandcardNum() - shownum)*0.5
			else
				return num + (player:getHandcardNum() - shownum)*0.6
			end
		elseif class_name == "Peach" then
			if player:hasSkills("fuyin_shayue") then	--有5点以下的福音算一个桃（注意不要写在下面的elseif里面，因为一个elseif .. then判断不完）
				local used_numbers = {}
				for i = 1,13,1 do
			...
			elseif player:hasSkills("chunlao|luazuijiao") then
				return num + player:getPile("wine"):length()
			elseif player:hasSkills("luazhafu") and player:getMark("@luazhafu") > 0 then	--有符算一个桃
				return num + 1
			elseif player:hasSkills("yanling") and player:getMark("@yanling") > 0 and for_others then	--有糕点算一个桃
				return num + 1
			elseif player:hasSkills("shisui") and player:getMaxHp() > 0 and for_others then	--有逝随算一个桃
				return num + 1
		...
		elseif class_name == "Analeptic" then
			...
			elseif player:hasSkills("jiushi|luajiushi") then
				return num + 1
			elseif player:hasSkill("yizan") then
				return num + basiccard + (player:getHandcardNum() - shownum)*0.5
			elseif player:hasSkill("quanneng") and player:getMark("quanneng_used") == 0 and basiccard > 0 then
		...
		elseif class_name == "Nullification" then
			...
			elseif player:hasSkill("luaposhi") then
				return num + rednull + (player:getHandcardNum() - shownum)*0.5
...
function SmartAI:getCardsNum(class_name, flag, selfonly)
    ...
    if card_str then
        card_str = sgs.Card_Parse(card_str)
        if card_str then
            if card_str:getSkillName() == "spear" or card_str:getSkillName() == "fuhun" or card_str:getSkillName() == "mingguang" then
            ...
            elseif card_str:getSkillName() == "luazuijiao" then
                n = n + player:getPile("wine"):length() - 1
...
function SmartAI:getAllPeachNum(player, ignore_skills)
    player = player or self.player
    local n = 0
    for _, friend in ipairs(self:getFriends(player)) do
        local num = self.player:objectName() == friend:objectName() and self:getCardsNum("Peach") or getCardsNum("Peach", friend, self.player, ignore_skills, true)
        n = n + num
    end
    return n
end
...
function SmartAI:useBasicCard(card, use)
    ...
    if not (card:isKindOf("Peach") and self.player:getLostHp() > 1) and self:needBear() and not self:needBear(self.player, true) then return end
    --if not (card:isKindOf("Peach") and self.player:getLostHp() > 1) and self:needBearCard(card) then return end
...
function SmartAI:aoeIsEffective(card, to, source)
    ...
    if (source:hasSkill("wuyan") and to:hasSkill("wuyan")) and not source:hasSkills("jueqing|lualiezhi") and not to:hasSkills("lualiezhi") then
        return false
    end
    ...
    if to:hasSkills("danlao|cangcong") and #players > 2 then
        return false
    end
    ...
    if not self:hasTrickEffective(card, to, source) or not self:damageIsEffective(to, sgs.DamageStruct_Normal, source, card) then
        return false
    end
    return true
end
...
function SmartAI:getDistanceLimit(card, from)
    ...
    if (card:isKindOf("Snatch") or card:isKindOf("SupplyShortage")) and card:getSkillName() ~= "qiaoshui" then
		local x = 1
		if card:isKindOf("SupplyShortage") and from:hasSkills("duanliang") then
			x = 2
		end
        return x + sgs.Sanguosha:correctCardTarget(sgs.TargetModSkill_DistanceLimit, from, card)
...
function SmartAI:exclude(players, card, from)
    ...
    for _, player in ipairs(players) do
        if not self.room:isProhibited(from, player, card) then
            ...
            if limit then
                should_insert = (from:distanceTo(player, range_fix) <= limit) or (card:isKindOf("SupplyShortage") and from:hasSkill("ol_duanliang") and from:getHandcardNum() <= player:getHandcardNum())
            end
...
function SmartAI:getAoeValueTo(card, to, from)
    ...
    if self:aoeIsEffective(card, to, from) then
		...
		--不可响应部分
		if from:hasSkill("santan") and from:getMark("santan_counter") == 2 then	--三叹不可响应
			sj_num = 0
		end
		if from:getPile("moci"):length() > 0 and from:getPile("moci"):length() > to:getHandcardNum() then	--魔刺不可响应
			sj_num = 0
		end
        ...
        if card:isKindOf("ArcheryAttack") then
            ...
            elseif to:hasSkills("xiaoan") and (sj_num >= 1 or self:hasEightDiagramEffect(to)) and table.contains(self:findPlayerToDamage(1, to, sgs.DamageStruct_Thunder, self.room:getAlivePlayers(), true, 10, true), from) then
                value = value + 100 + to:getHandcardNum()*35
        ...
        if not from:hasSkills("jueqing|lualiezhi") and not to:hasSkills("lualiezhi") then
            ...
            if to:getHp() > 1 then
                if to:hasSkills("quanji|luaquanji_jie") then value = value + 10 end
                if to:hasSkill("langgu") and self:isEnemy(to, from) then value = value - 15 end

                if to:hasSkills("jianxiong|luajianxiong|nitai|fanli") then
        ...
        if to:hasSkill("cangcong") and self.player:aliveCount() > 2 then value = value + 20 end
    end
...
function getLord(player)
    ...
    if mode == "06_3v3" or mode == "04_if" then
...
function SmartAI:getAoeValue(card, player)
    ...
    if isEffective_F == 0 and isEffective_E == 0 then
        return attacker:hasSkills("jizhi|nosjizhi|zhuoshi|qilv|zhanshu") and 10 or -100
    ...
    if attacker:hasSkills("nosjizhi|jizhi|zhuoshi|qilv|zhanshu") then
        forbid_start = false
        good = good + 51
    end
    ...
    if attacker:hasSkills("jianxiong|luajianxiong|luanji|mobileluanji|qice|manjuan|nitai|fanli") then good = good + 2 * enemy_number end
...
function SmartAI:hasTrickEffective(card, to, from, ignore_skills)
    from = from or self.room:getCurrent()
    to = to or self.player
	...
	if not ignore_skills then
		...
		if to:hasSkill("luajiqiao") and card:getSuit() == sgs.Card_Spade then return false end
		if to:hasSkill("fagun") and to:getMark("@fagun_effect") > 0 and card:isRed() and (card:isKindOf("Duel") or card:isKindOf("FireAttack") or card:isKindOf("ArcheryAttack") or card:isKindOf("SavageAssault") or card:isKindOf("Drowning")) then return false end	--法棍红伤害锦囊无效
	
		if (from:hasSkill("wuyan") or to:hasSkill("wuyan")) and not from:hasSkills("jueqing|lualiezhi") and not to:hasSkills("lualiezhi") then
		...
		if to:hasSkill("yujie") and to:getMark("&yujie+_lun!") < 3 then	--域界
			local can_provide_trick = false
			for _, c in sgs.qlist(from:getHandcards()) do
				if c:isKindOf("TrickCard") and c:getEffectiveId() ~= card:getEffectiveId() and not c:isOvert() then
					can_provide_trick = true
					break
				end
			end
			if not can_provide_trick then
				return false
			end
		end
	end
...
function SmartAI:useTrickCard(card, use)
    if not card then global_room:writeToConsole(debug.traceback()) return end
    if self.player:hasSkill("ytchengxiang") and self.player:getHandcardNum() < 8 and card:getNumber() < 7 then return end
    if self:needBear(self.player, false) and not ("amazing_grace|ex_nihilo|snatch|iron_chain|collateral"):match(card:objectName()) then return end
    if self:needBear(self.player, true) then return end
    --if self:needBearCard(card) then return end
    ...
    if card:isKindOf("AOE") then
        ...
        if self.player:hasSkill("noswuyan")
            or (self.player:hasSkill("wuyan") and not self.player:hasSkills("jueqing|lualiezhi")
                and not (menghuo and (not menghuo:hasSkill("wuyan") or menghuo:hasSkills("jueqing|lualiezhi")) and avail > 1)) then
        ...
        if self:hasSkills("luanji|mobileluanji") and self.player:isLord() and sgs.turncount < 2 then
    ...
    if use.to then
        if not use.to:isEmpty() and sgs.dynamic_value.damage_card[card:getClassName()] then
            ...
            for _, target in sgs.qlist(use.to) do
                ...
                if self:damageIsEffective(target, nature, self.player, card) then return end
            end
            use.card = nil
        end
    end
...
function SmartAI:hasEightDiagramEffect(player)
    player = player or self.player
    return player:hasArmorEffect("eight_diagram") or player:hasArmorEffect("bazhen|xiangrui")
end

function SmartAI:hasCrossbowEffect(player)
    player = player or self.player
    return player:hasWeapon("crossbow") or player:hasSkills("paoxiao|luapaoxiao|xingxiong")
end
...
function SmartAI:evaluateWeapon(card, player, target)
    ...
    if card:isKindOf("Crossbow") and not player:hasSkills("paoxiao|luapaoxiao|xingxiong") and inAttackRange then
    ...
    if player:hasSkills("weiliu") then deltaSelfThreat = deltaSelfThreat + 0.5 end
...
function SmartAI:evaluateArmor(card, player)
    ...
    if player:hasSkills("weiliu") then value = value + 0.5 end
	if player:getArmor() and ecard == player:getArmor() and ecard:isKindOf("Neneko") and player:getMark("&neneko_record_basic") + player:getMark("&neneko_record_trick") > 0 then
		value = value + 2
	end
...
function SmartAI:useEquipCard(card, use)
    if not card then global_room:writeToConsole(debug.traceback()) return end
	if not card:isAvailable(self.player) or self.room:isProhibited(self.player, self.player, card) then return end
	...
	if card:isKindOf("Weapon") then		--溢出判定前的特殊处理
		if self.player:hasSkill("jvhe") then	--居合，手里有能用且要用的杀时，不使用手牌中的武器
			for _, c in sgs.qlist(self.player:getHandcards()) do
				if c:isKindOf("Slash") and self.player:canUse(c) and self:willUse(self.player, c) then
					return
				end
			end
		end
	...
	if card:isKindOf("EightDiagram") and self.player:hasSkills("xiangrui") then	--有祥瑞就不要装八卦了
		return
	end
	
    if not self:needBear() and self:getOverflow() > 0 and (not card:isKindOf("Armor") or self:evaluateArmor(card) > self:evaluateArmor()) then	--溢出就用
        use.card = card
        return
    end
	if self:needBear(self.player, true) then
		return
	end
    ...
    if same then
        if (self:hasSkill("nosgongqi") and self:slashIsAvailable())
            ...
            or (self.player:hasSkills("qixi|duanliang|ol_duanliang|yinling") and (card:isBlack() or same:isBlack()))
            ...
            or (self.player:hasSkill("weiliu") and self:getOverflow() <= 1)
    ...
    if card:isKindOf("Weapon") then
        ...
        if self:hasSkills("paoxiao|luapaoxiao|nosfuhun|xingxiong", self.player) and card:isKindOf("Crossbow") then return end
        ...
        if self.player:hasSkill("qisheng")
            and not self.player:hasUsed("#qisheng") and self.player:getWeapon() and not card:isKindOf("Crossbow") then return end
    ...
    elseif card:isKindOf("Armor") then
        ...
        --if self:needBearCard(card) and self:getOverflow() <= 1 and self.player:getLostHp() == 0 then return end
        ...
        if lion and self.player:isWounded() and not self.player:hasArmorEffect("silver_lion") and not card:isKindOf("SilverLion")
            and not (self:hasSkills("bazhen|xiangrui|yizhong|bingshen") and not self.player:getArmor()) then
            use.card = lion
            return
        end
        ...
        if (self:evaluateArmor(card) > self:evaluateArmor() or isenemy_zzzh) and (self:getOverflow() > 0 or not same) then use.card = card end
        return
    elseif self:needBear() then return
    --elseif self:needBearCard(card) then return
    ...
    elseif card:isKindOf("DefensiveHorse") then
        local tiaoxin = true
        if self.player:hasSkills("tiaoxin") then
        ...
        if self.player:hasSkills("luatiaoxin_jie") then
            local dummy_use = { isDummy = true, defHorse = true }
            self:useSkillCard(sgs.Card_Parse("#luatiaoxin_jie:.:"), dummy_use)
            if not dummy_use.card then tiaoxin = false end
        end
...
function getBestHp(player)
    ...
    if player:hasSkill("shenghua") and player:getMark("shenghua") == 0 then return 2 end
	if player:hasSkill("luadaizhe") then return 2 end
    if player:hasSkill("milian") and player:getMark("milian") == 0 then	--秘恋，未觉醒最佳体力值最少为4（前两轮为3）
		if player:hasSkill("qicheng") and player:getRoom():getTag("TurnLengthCount"):toInt() <= 2 then
			return 3
		else
			return math.max(4, player:getHp())
		end
	end
    ...
    if (player:hasSkills("quanji+zili") or player:hasSkills("luaquanji_jie+zili")) and player:getMark("zili") == 0 then return (player:getMaxHp() - 1) end
    if player:getMark("&gehun!") > 0 then return player:getMaxHp()-player:getMark("&gehun!") end	--鸽魂，时机体力上限为体力上限-鸽魂计数
    return math.max(player:getMaxHp(), player:getHp())
...
function SmartAI:needToLoseHp(to, from, isSlash, passive, recover)
    from = from or self.room:getCurrent()
    to = to or self.player
    if isSlash and not from:hasSkills("jueqing|lualiezhi") and not (to and to:hasSkills("lualiezhi")) then
        if from:hasSkill("nosqianxi") and from:distanceTo(to) == 1 and not self:isFriend(from, to) then
            return false
        end
        if (from:hasWeapon("ice_sword") or (self.is_ice_slash and not self.ignore_ice)) and to:getCards("he"):length() > 1 and not self:isFriend(from, to) then
            return false
        end
        if from:hasSkills("bingxin") and to:getCards("he"):length() > 1 and not self:isFriend(from, to) then
            return false
    ...
    if (from:hasSkills("jueqing|lualiezhi") or to:hasSkills("lualiezhi")) and self:hasSkills(sgs.masochism_skill, to) then return false end
    ...
    if not passive then
        if to:getMaxHp() > 2 then
			...
			--if to:hasSkills("luafenyin") then n = math.min(n, to:getMaxHp() - 1) end
			if to:hasSkills("xianwei|newfuyu|luafenyin") then n = math.min(n, to:getMaxHp() - 1) end	--衔尾/新抚育/奋音，需要受伤
			if to:hasSkills("xinrong") and to:getMark("&xinrong!") == 0 then n = math.min(n, to:getMaxHp() - 1) end	--欣荣，需要受伤（排除欣荣的伪满血情况）
			if to:hasSkills("yongning") and not to:isKongcheng() then n = math.min(n, 1) end	--永宁，非空城需要1血
			if to:hasSkills("youlian") then n = math.min(n, 1) end	--幽炼，需要1血
			if to:hasSkills("shixi") then	--势惜，满足条件则需要失去至少2血
				local kingdoms = {}
				for _,p in sgs.qlist(self.room:getAlivePlayers()) do
					if p:isWounded() and not table.contains(kingdoms, p:getKingdom()) then
						table.insert(kingdoms, p:getKingdom())
				...
				local X = #kingdoms
				if X > 1 then
					n = math.min(n, to:getMaxHp() - 2)
				end
			end
			if to:hasSkills("zhongyu") and to:getMark("zhongyu_damage_counter") < 6 and self.room:getTag("TurnLengthCount"):toInt() < 6 then n = math.min(n, 1) end	--终语，凑齐6次伤害或第6轮前需要1血
			if to:hasSkills("shushen") then
				for _, card in sgs.qlist(to:getCards("hej")) do
					if card:isKindOf("Peach") or (not card:isKindOf("BasicCard") and to:hasSkills("luashenzhi_x")) and #self:getFriendsNoself(to) > 0 then
						n = math.min(n, to:getMaxHp() - 1)
					end
				end
			end
			if to:hasSkills("luaxingzhao") then
				local injured_count = 0
				for _, p in sgs.qlist(self.room:getAlivePlayers()) do
					if p:isWounded() then
						injured_count = injured_count + 1
			...
			if to:hasSkills("luadaizhe") then n = math.min(n, 2) end
...
function SmartAI:needToThrowArmor(player, throw_armor_area)
    player = player or self.player
    if not player:getArmor() or (not player:hasArmorEffect(player:getArmor():objectName()) and player:getArmor():objectName() ~= "small_silver_lion") then return false end
    if self:isEquipLocking(player) then return false end
	if player:hasSkills("bazhen|yizhong") and not player:getArmor():isKindOf("EightDiagram") and not throw_armor_area then return true end
	if player:hasSkills("bingshen") and not player:getArmor():isKindOf("RenwangShield") and not throw_armor_area then return true end
    if player:hasSkills("xiangrui") and player:getArmor() and not throw_armor_area then return true end
    if self:evaluateArmor(player:getArmor(), player) <= -2 then return true end
    if player:hasArmorEffect("silver_lion") and player:getLostHp() > 0 and not self:needToLoseHp(player, player, false, false, true) then
    ...
    if not self.player:hasSkills("moukui|longxi") and player:hasArmorEffect("vine") and player:objectName() ~= self.player:objectName() and self:isEnemy(player)
        ...
        and (self:getCard("FireSlash") or (self:getCard("Slash") and (self.player:hasWeapon("fan") or self.player:hasSkills("lihuo|zonghuo|fenxin_S|quanneng") or self:getCardsNum("fan") >= 1)))
...
function SmartAI:getTemporaryCards(to)	--获得角色的所有临时牌的id列表
	if not to then to = self.player end
	local has_zhonggong = false
	for _, friend in ipairs(self:getFriends(to)) do
		if friend:hasSkill("zhonggong") then
			has_zhonggong = true
			break
		end
	end
    local temporary_cards = {}
    for _, card in sgs.qlist(to:getCards("he")) do
		local id = card:getEffectiveId()
		if not table.contains(temporary_cards, id) then
			if to:getMark("huawu_"..id) > 0 then
				table.insert(temporary_cards, id)
			elseif to:getMark("shengguang_card_"..id) > 0 then
				table.insert(temporary_cards, id)
			elseif to:getMark("xinrong_card_"..id) > 0 then
				table.insert(temporary_cards, id)
			elseif to:hasSkill("yangneng") and to:getMark("yangneng_card_"..id) > 0 then	--注意前提必须持有阳能，否则所有角色都会受影响！
				table.insert(temporary_cards, id)
			elseif has_zhonggong and (card:isKindOf("Xuanwujia") or card:isKindOf("Xueniangaogun")) then	--队友有重工，雪年糕棍和玄武甲随便丢
				table.insert(temporary_cards, id)
			end
		end
	end
...
function SmartAI:hasTemporaryCard(to)	--判断角色是否有临时牌
	if not to then to = self.player end
	local temp_cards = self:getTemporaryCards(to)
...
function SmartAI:isTemporaryCard(to, card_id)	--根据id判断角色的某张牌是否是临时牌
	if not to then to = self.player end
	local temp_cards = self:getTemporaryCards(to)
	if temp_cards and #temp_cards > 0 then
		return table.contains(temp_cards, card_id)
	end
	return false
end

function SmartAI:needToThrowHandcard(player, n, method)	--判断某角色是否需要以某种方式失去n张手牌
    player = player or self.player
	...
	if player:hasSkill("xindong") and player:getMark("xindong_count") < 2 and method == sgs.CardMoveReason_S_REASON_DISCARD then	--心动
		return true
	end
	if player:hasSkill("zijian") and not self:isWeak(player) and method == sgs.CardMoveReason_S_REASON_DISCARD then	--自荐非虚弱
		return true
	end
	return false
end
...
function SmartAI:doNotDiscard(to, flags, conservative, n, cant_choose)
    ...
    if to:hasSkills("tuntian|luatuntian_jie|luatuntian_jieh") and to:hasSkill("zaoxian") and to:getPhase() == sgs.Player_NotActive and (conservative or #self.enemies > 1) then return true end

    if cant_choose then
        if to:hasSkill("lirang|zijian") and #self.enemies > 1 then return true end
        if to:hasSkill("sanre") and not self:isWeak(to) then return true end
        ...
        elseif flags == "e" or (flags == "he" and to:isKongcheng()) then
			...
			if self:isEquipLocking(to) then return true end
...
function SmartAI:findPlayerToDiscard(flags, include_self, isDiscard, players, return_table, from)
	from = from or self.player
    ...
    if not players then
        ...
        for _, player in sgs.qlist(players) do
            if self:isFriend(player, from) and (include_self or player:objectName() ~= from:objectName()) then table.insert(friends, player)
            elseif self:isEnemy(player, from) then table.insert(enemies, player) end
        end
    end
    flags = flags or "he"
    ...
    if flags:match("e") then
        for _, enemy in ipairs(enemies) do
            if from:canDiscard(enemy, "e") and not self:isEquipLocking(enemy) then
                local dangerous = self:getDangerousCard(enemy)
                if dangerous and (not isDiscard or from:canDiscard(enemy, dangerous)) then
                    table.insert(player_table, enemy)
                end
            end
        end
        for _, enemy in ipairs(enemies) do
            if not self:isEquipLocking(enemy) and enemy:hasArmorEffect("eight_diagram") and enemy:getArmor() and not self:needToThrowArmor(enemy) and from:canDiscard(enemy, enemy:getArmor():getEffectiveId()) then
                table.insert(player_table, enemy)
            end
        end
    end
    ...
    if flags:match("j") then
        for _, friend in ipairs(friends) do
            if ((friend:containsTrick("indulgence") and not friend:hasSkill("keji")) or friend:containsTrick("supply_shortage"))
                and not friend:containsTrick("YanxiaoCard") and not (friend:hasSkills("qiaobian|fahun") and not friend:isKongcheng())
                and (not isDiscard or from:canDiscard(friend, "j")) then
                table.insert(player_table, friend)
            end
        end
        for _, friend in ipairs(friends) do
            if friend:containsTrick("lightning") and self:hasWizard(enemies, true) and (not isDiscard or from:canDiscard(friend, "j")) then table.insert(player_table, friend) end
        end
        for _, enemy in ipairs(enemies) do
            if enemy:containsTrick("lightning") and self:hasWizard(enemies, true) and (not isDiscard or from:canDiscard(enemy, "j")) then table.insert(player_table, enemy) end
        end
    end

    if flags:match("e") then
        for _, friend in ipairs(friends) do
            if self:needToThrowArmor(friend) and (not isDiscard or from:canDiscard(friend, friend:getArmor():getEffectiveId())) then
                table.insert(player_table, friend)
            end
        end
        for _, enemy in ipairs(enemies) do
            if from:canDiscard(enemy, "e") and not self:isEquipLocking(enemy) then
                ...
                if valuable and (not isDiscard or from:canDiscard(enemy, valuable)) then
                    table.insert(player_table, enemy)
                end
            end
        end
        for _, enemy in ipairs(enemies) do
            if enemy:hasSkills("jijiu|beige|mingce|weimu|qingcheng") and not self:doNotDiscard(enemy, "e") and not self:isEquipLocking(enemy) then
                if enemy:getDefensiveHorse() and (not isDiscard or from:canDiscard(enemy, enemy:getDefensiveHorse():getEffectiveId())) then table.insert(player_table, enemy) end
                if enemy:getArmor() and not self:needToThrowArmor(enemy) and (not isDiscard or from:canDiscard(enemy, enemy:getArmor():getEffectiveId())) then table.insert(player_table, enemy) end
                if enemy:getOffensiveHorse() and (not enemy:hasSkill("jijiu") or enemy:getOffensiveHorse():isRed()) and (not isDiscard or from:canDiscard(enemy, enemy:getOffensiveHorse():getEffectiveId())) then
                    table.insert(player_table, enemy)
                end
                if enemy:getWeapon() and (not enemy:hasSkill("jijiu") or enemy:getWeapon():isRed()) and (not isDiscard or from:canDiscard(enemy, enemy:getWeapon():getEffectiveId())) then
                    table.insert(player_table, enemy)
                end
            end
        end
    ...
    if flags:match("h") then
        for _, enemy in ipairs(enemies) do
            ...
            local flag = string.format("%s_%s_%s","visible", from:objectName(), enemy:objectName())
            if #cards <= 2 and not enemy:isKongcheng() and not (((enemy:hasSkills("tuntian|luatuntian_jie|luatuntian_jieh") and enemy:hasSkills("zaoxian|luafengchan")) or enemy:hasSkills(sgs.lose_card_skills)) and enemy:getPhase() == sgs.Player_NotActive) then
                for _, cc in ipairs(cards) do
                    if (cc:hasFlag("visible") or cc:hasFlag(flag)) and (cc:isKindOf("Peach") or cc:isKindOf("Analeptic")) and (not isDiscard or from:canDiscard(enemy, cc:getId())) then
                        table.insert(player_table, enemy)
                    end
                end
            end
    ...
    if flags:match("e") then
        for _, enemy in ipairs(enemies) do
            if enemy:hasEquip() and not self:doNotDiscard(enemy, "e") and not self:isEquipLocking(enemy) and (not isDiscard or from:canDiscard(enemy, "e")) then
                table.insert(player_table, enemy)
            end
        end
    end
    ...
    if flags:match("j") then
        for _, enemy in ipairs(enemies) do
            if enemy:containsTrick("YanxiaoCard") and (not isDiscard or from:canDiscard(enemy, "j")) then table.insert(player_table, enemy) end
        end
    end
    ...
    if flags:match("h") then
        ...
        for _, enemy in ipairs(enemies) do
            if (not isDiscard or from:canDiscard(enemy, "h")) and not self:doNotDiscard(enemy, "h") then
                table.insert(player_table, enemy)
            end
        end
    end
    ...
    if flags:match("h") then
        local zhugeliang = self.room:findPlayerBySkillName("kongcheng")
        if zhugeliang and self:isFriend(zhugeliang, from) and zhugeliang:getHandcardNum() == 1 and self:getEnemyNumBySeat(from, zhugeliang) > 0
            and zhugeliang:getHp() <= 2 and (not isDiscard or from:canDiscard(zhugeliang, "h")) then
...
function hasManjuanEffect(player)	--从bgm-ai.lua搬运
	if (player:hasSkill("manjuan") and player:getPhase() == sgs.Player_NotActive) or player:getMark("&yinbao") > 0 or (player:hasSkill("yangneng") and player:getPhase() <= sgs.Player_Play) then
		return true
	end
	return false
end

function SmartAI:hasZhenguEffect(to, from)	--yun
...
function SmartAI:hasJieyingyEffect(player)	--yun
    player = player or self.player
	if player:getMark("@thiefed") > 0 and not player:hasSkill("jieyingy") then 
		local shenganning = self.room:findPlayerBySkillName("jieyingy")
		if shenganning and not self:isFriend(shenganning, player) then return true end
	end
	if player:getMark("fushen_target") > 0 then 	--被附身时视作被劫营状态（不主动摸牌）
		local from = self.room:findPlayerBySkillName("fushen")
		if from and not self:isFriend(from, player) then return true end
	end
	local gexuan = self.room:findPlayerBySkillName("xinlifu")
	if gexuan and player:getMark(player:objectName().."xinlifu") > 0 then if not self:isFriend(gexuan, player) then return true end end
	return false
end

function SmartAI:needDraw(to, notDraw)	--yun
	if not to then return false end
	if (to:hasSkills("manjuan|zishu") or hasManjuanEffect(to)) and to:getPhase() ~= sgs.Player_NotActive then return true end
...
function SmartAI:canDraw(to, from)	--yun
	--if not to then return false end
	to = to or self.player
...
function hasYinshiEffect(to, hasArmor)	--yun
	if hasArmor then return to:hasSkill("yinshi") and to:getMark("@dragon") + to:getMark("@phoenix") == 0 end
	return to:hasSkill("yinshi") and to:getMark("@dragon") + to:getMark("@phoenix") == 0 and not to:getArmor()
end

function hasBaimeiEffect(to)	--yun
	return to:hasSkill("twyj_baimei") and to:isKongcheng()
end

function hasOrangeEffect(player)	--yun
	local luji = global_room:findPlayerBySkillName("huaiju")
	if not luji then return false end
	return player:getMark("@orange") > 0
end

function hasTuntianEffect(to)	--yun
	return to:hasSkills("tuntian|beyond_tuntian") and to:getPhase() == sgs.Player_NotActive
end

function hasFuyinEffect(to, from, beforeUse)	--yun
	if beforeUse then return to:hasSkill("fuyin_sec_rev") and to:getMark("fuyin_sec_rev-Clear") == 0 and from:getHandcardNum() > to:getHandcardNum() end
	return to:hasSkill("fuyin_sec_rev") and to:getMark("fuyin_sec_rev-Clear") == 0 and from:getHandcardNum() >= to:getHandcardNum()
end

function hasCihuaiEffect(to)	--yun
	return to:hasSkill("cihuai") and to:getMark("@cihuai") > 0 and (to:hasWeapon("crossbow") or to:hasSkills("paoxiao|luapaoxiao|xingxiong"))
end

function hasGuanchaoEffect(to)	--yun
	return to:hasSkill("guanchao") and to:getPhase() == sgs.Player_Play and to:getMark("guanchao_break_replay") == 0 
end

function hasBenxiEffect(from)	--yun
	if from:hasSkills("benxi|gaibenxi") and from:getPhase() ~= sgs.Player_NotActive then
		local can = true
		for _, p in sgs.qlist(global_room:getOtherPlayers(from)) do
...
function SmartAI:findPlayerToDraw2(include_self, drawnum, players, pay, notDraw, return_table)	--新函数，更智能
    drawnum = drawnum or 1
    local player_table = {}
	if not players then
        players = include_self and self.friends or self.friends_noself
    end
	
    local friends = {}
    for _, player in ipairs(players) do
        if self:isFriend(player) and not hasManjuanEffect(player) and not (self:needKongcheng(player, true) and drawnum <= 2)	--yun
		and not self:hasZhenguEffect(player) then
			table.insert(friends, player)
        end
    end
    if #friends > 0 then
		...
		if drawnum >= 2 then
			for _, friend in ipairs(friends) do
				if friend:hasSkill("hongde") then
					if return_table then
						if not table.contains(player_table, friend) then table.insert(player_table, friend) end
					else return friend
					end
				end
			end
		...
		for _, friend in ipairs(friends) do
			if friend:isKongcheng() and not self:needKongcheng(friend, true) and friend:hasSkills(sgs.dont_kongcheng_skill) then
				if return_table then
					if not table.contains(player_table, friend) then table.insert(player_table, friend) end
				else return friend
				end
		...
		for _, friend in ipairs(friends) do
			if self:needDraw(friend, notDraw) then
				if return_table then
					if not table.contains(player_table, friend) then table.insert(player_table, friend) end
				else return friend
				end
		...
		for _, friend in ipairs(friends) do
			if friend:getHandcardNum() < 2 and not self:needKongcheng(friend) and not self:willSkipPlayPhase(friend) 
			and not friend:hasSkills(sgs.need_kongcheng.."|tianming|fangquan") then
				if return_table then
					if not table.contains(player_table, friend) then table.insert(player_table, friend) end
				else return friend
				end
		...
		local AssistTarget = self:AssistTarget()
		if AssistTarget and not self:willSkipPlayPhase(AssistTarget) and not AssistTarget:hasSkills(sgs.need_kongcheng.."|tianming|fangquan")
		and (AssistTarget:getHandcardNum() < AssistTarget:getMaxCards() * 2 or AssistTarget:getHandcardNum() < self.player:getHandcardNum()) then
			for _, friend in ipairs(friends) do
				if friend:objectName() == AssistTarget:objectName() and not self:willSkipPlayPhase(friend) then
					if return_table then
						if not table.contains(player_table, friend) then table.insert(player_table, friend) end
		...
		for _, friend in ipairs(friends) do
			if not self:needKongcheng(friend) and not self:willSkipPlayPhase(friend) and not friend:hasSkill("fangquan") then
				if return_table then
					if not table.contains(player_table, friend) then table.insert(player_table, friend) end
				else return friend
				end
		...
		if not pay then
			for _, friend in ipairs(friends) do	--乐摸牌
				if friend then
					if return_table then
						if not table.contains(player_table, friend) then table.insert(player_table, friend) end
					else return friend
	...
	if not pay then
		for _, player in ipairs(players) do	--自书摸牌
			if self:isFriend(player) and not (player:hasSkill("manjuan") and player:getPhase() == sgs.Player_NotActive) and not (self:needKongcheng(player, true) and drawnum <= 2)
			and not self:hasZhenguEffect(player) then
				if return_table then
					if not table.contains(player_table, friend) then table.insert(player_table, friend) end
				else return friend
...
function SmartAI:findPlayerToSlash(distance, slash, targets, must)	--yun
	...
	if targets then 
		...
		if not distance then
			...
			for _, to in sgs.qlist(self.room:getOtherPlayers(self.player)) do
				if self.player:inMyAttackRange(to) then table.insert(tos, to) end
			end
		end
	end
	if #tos < 1 then
	...
	local pri, enemy, friend, other
	
	for _, to in ipairs(tos) do
		if self:isFriend(to) and self:isPriorFriendOfSlash(to, slash) then pri = to end
		if not enemy and self:isEnemy(to) and self:slashIsEffective(slash, to) and not self:slashProhibit(slash, to) 
		and (self:canDamage(to, self.player, true) or (to:isLord() and sgs.turncount < 2 and #self.enemies < 2)) then
			enemy = to
		end
		if self:isFriend(to) and self:slashIsEffective(slash, to) and self:canDamage(to, self.player, true) then
			friend = to
		end
		if not self:slashIsEffective(slash, to) then other = to end
	end
	if is_clone_card then
		slash:deleteLater()
	end
	if pri or enemy or friend then return pri or enemy or friend end
	if not must then return nil end
    ...
    local function getDamageValue(target, self_only)
        ...
        if self:damageIsEffective(target, nature, source) then
			...
			elseif nature == sgs.DamageStruct_Thunder then
				if target:hasArmorEffect("toujing") then
					count = count + 1
				end
            end
        ...
        if count > 0 then
            ...
            if deathFlag then
                ...
                if hp <= count then
                    if target:hasSkills("jiushi|luajiushi") and target:faceUp() then
                        value = value + 25
                    end
                end
                if self:getDamagedEffects(target, source) then
                    ...
                    if target:hasSkill("luatuifeng") then
                        value = value + 18
                    end
                end
            end
...
function SmartAI:dontRespondPeachInJudge(judge)
    ...
    if (judge.reason == "tuntian" or judge.reason == "luatuntian_jie" or judge.reason == "luatuntian_jieh") and judge.who:getMark("zaoxian") == 0 and judge.who:getPile("field"):length() < 2 then return true
    elseif (judge.reason == "EightDiagram" or judge.reason == "bazhen" or judge.reason == "xiangrui") and
    ...
    --elseif judge.reason == "luaxiaoxi" then return true
...
function SmartAI:findFriendsByType(prompt, player)
    ...
    if prompt == sgs.Friend_Draw then
        for _, friend in ipairs(friends) do
            if not friend:hasSkill("manjuan") and not hasManjuanEffect(friend) and not self:needKongcheng(friend, true) then return true end
        end
...
function hasBuquEffect(player)
    return (player:hasSkill("buqu") and player:getPile("trauma"):length() <= 4) or (player:hasSkill("nosbuqu") and player:getPile("nostrauma"):length() <= 4)
end

function SmartAI:needBearCard(card, player, fatal)	--新函数，输入card和player，返回player是否不使用card；fatal为真代表是紧急情况（如需要濒死救人等），会无视一些负面，除非真的不能用
    player = player or self.player
	
	--若此牌有花色，检查是否被月醴限制
	if card:getSuit() >= 0 and card:getSuit() <= 3 then
		for _, p in sgs.qlist(self.room:findPlayersBySkillName("yueli")) do
			if p:getMark("&yueli+"..card:getSuitString().."_char") > 0 and (not fatal or player:getHp() == 1) then
				return true
			end
		end
	end
	...
	--蜜糕牌，检查场上是否有未觉醒的迷食，有则不使用，与其敌对则反之
	if card:hasFlag("&migao") and not fatal then
		local has_mishi = false
		local is_enemy = false
		for _, p in sgs.qlist(self.room:findPlayersBySkillName("mishi")) do
			if p:getMark("mishi") == 0 then
				has_mishi = true
				is_enemy = self:isEnemy(p, player)
				break
			end
		end
		if is_enemy then
			has_mishi = not has_mishi
		end
		return has_mishi
	end
	...
	if player:hasSkill("wenyu") and card:isKindOf("Slash") and player:getPhase() ~= sgs.Player_NotActive --[[and not fatal]] then
		...
		local maxcard = self:getOverflow(self.player, true)
	...
	--应势，出牌阶段存红桃
	--[[if card:getSuit() == sgs.Card_Heart and player:hasSkills("luayingshi") and player:getPhase() == sgs.Player_Play then
		local count
		count = 0
		for _, cd in sgs.qlist(player:getCards("h")) do
		...
		if (count == 1 or (self:getOverflow() <= 0)) and not card:isKindOf("EquipCard") then	--存住手牌唯一的红桃，或不溢出则存（允许装红桃装备）
			return true
		end
	end
	
	--征荣（蓄单张征荣牌）
	if player:hasSkill("luazhengrong") and math.mod(player:getMark("luazhengrong_count"), 2) == 0 and player:getPhase() == sgs.Player_Play then
		local zhengrong_count = 0
		local thiscard = false
		for _, c in sgs.qlist(player:getHandcards()) do
			if c:isKindOf("Slash") or c:isKindOf("AOE") or c:isKindOf("AmazingGrace") or c:isKindOf("Collateral") or c:isKindOf("Dismantlement") or c:isKindOf("Duel") or c:isKindOf("FireAttack") or c:isKindOf("GodSalvation") or c:isKindOf("Indulgence") or c:isKindOf("IronChain") or c:isKindOf("Snatch") or c:isKindOf("SupplyShortage") then
				zhengrong_count = zhengrong_count + 1
				if card:getId() == c:getId() then
					thiscard = true
				end
			end
		end
		if thiscard and zhengrong_count == 1 then
			return true
		end
	end]]
...
function SmartAI:getBestKeepHandcardNum(player, default_value)	--新函数，输入player，返回player的当前最优保留手牌数（需要用到剩下几张），若未被技能影响则返回default_value
    player = player or self.player
	if player:hasSkills("luadanxin") and player:getPhase() ~= sgs.Player_NotActive then	--殚心，回合内把牌用到少于体力值
		return math.max(0, player:getHp() - 1)
	end
	if player:hasSkills("luaqingcheng") and player:getPhase() ~= sgs.Player_NotActive then	--倾城，回合内把牌用到少于体力值
		return math.max(0, player:getHp() - 1)
	end
	if player:hasSkills("zuoye|ciyuangongzhen") and player:getPhase() ~= sgs.Player_NotActive then	--作业/次元共振，回合内把牌用到少于体力值
		return math.max(0, player:getHp() - 1)
	end
	if player:hasSkills("yueyin+xingyi") then	--月引+星移，不论回合内外都把牌用到少于体力值
		return math.max(0, player:getHp() - 1)
	end
	if player:hasSkills("pianpian") and 
		((player:getChangeSkillState("pianpian") <= 1 and player:getPhase() ~= sgs.Player_NotActive) or 
		 (player:getChangeSkillState("pianpian") == 2 and player:getPhase() == sgs.Player_NotActive)) then	--翩翩①/②，回合内/外把牌用到少于体力上限
		return math.max(0, player:getMaxHp() - 1)
	end
	if player:hasSkills("feizhai") and player:getPhase() ~= sgs.Player_NotActive then	--废宅，尽量回合内打空，但并不算需要空城所以写在这里
		return 0
	end
	if player:hasSkills("yahe") and player:getMark("&yahe!") == 1 and player:getPhase() ~= sgs.Player_NotActive then	--雅和，②生效中尽量把手牌用到全场最少
		...
		if profit + 1 > 0 then
			return min_handcard_num
		end
		return min_handcard_num - 1
	end
	...
	--	return 0
	--end
	if player:hasSkills("luamiaozuo") and player:getPhase() == sgs.Player_Play then	--玅佐，用到玅佐手牌数
		local best_card = player:getHandcardNum()
		local best_num = 0
		for i = 0,player:getHandcardNum(),1 do
			local count = 0
			for _, friend in ipairs(self.friends_noself) do
				if friend:getHandcardNum() == i and not (self:needKongcheng(friend) and friend:isKongcheng()) then
					count = count + 1
				end
			end
			if count >= best_num then
				best_num = count
				best_card = i
			end
		end
	...
	if player:getPhase() ~= sgs.Player_NotActive then	--友方有输粮，回合内把牌用到少于体力值
		local has_shuliang_friend = false
		for _, friend in ipairs(self.friends) do
			if friend:hasSkills("imba_shuliang") and friend:getPile("food"):length() > 0 then
				has_shuliang_friend = true
				break
			end
		end
		if has_shuliang_friend and not self:needBear(player, false, player:getHandcardNum() - player:getMaxCards()) then	--固定overflow值以免无限引用嵌套（needBear→getOverflow→getBestKeepHandcardNum→needBear...）
			return math.max(0, player:getHp() - 1)
		end
	end
	...
	return self:getOverflow(player, true)
end

function SmartAI:getFirstAttackTarget(player)	--新函数，扫描手牌中的伤害牌，获取使用价值最高的伤害牌要优先攻击的目标（返回值为SPlayerList）
	player = player or self.player
    local cards = sgs.QList2Table(player:getHandcards())
...
function SmartAI:isEquipLocking(player)	--新函数，判断某角色是否锁定了装备区
	player = player or self.player
	if player:hasSkills("gangbantuofeng") then
		return true
	elseif player:hasSkill("jiangzhuang") and player:getPhase() == sgs.Player_NotActive then
		return true
	end
	return false
end
