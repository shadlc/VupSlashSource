...
function SmartAI:findLeijiTarget(player, leiji_value, slasher, latest_version)
	if not latest_version and not player:hasSkills("leiji|nosleiji|olleiji|xiaoan") then
		return self:findLeijiTarget(player, leiji_value, slasher, 1) or self:findLeijiTarget(player, leiji_value, slasher, -1)
	end
	if not player:hasSkill(latest_version == 1 and "leiji" or "nosleiji") then
		if player:hasSkill("xiaoan") then
			latest_version = 2
		else
			return nil
		end
	...
	if slasher then
		...
		elseif latest_version == 1 then
			if not self:hasSuit("black", true, player) and player:getHandcardNum() < 2 then return nil end
		--elseif latest_version == 2 then
		--	if player:getHandcardNum() < 2 then return nil end
		end
	...
	local getCmpValue = function(enemy)
		...
		if enemy:hasSkill("hongyan") and latest_version ~= 2 then
		...
		if self:cantbeHurt(enemy, player, latest_version > 0 and 1 or 2) or self:objectiveLevel(enemy) < 3
			or (enemy:isChained() and not self:isGoodChainTarget(enemy, player, sgs.DamageStruct_Thunder, latest_version > 0 and 1 or 2)) then return 100 end
		...
		if enemy:isChained() and self:isGoodChainTarget(enemy, player, sgs.DamageStruct_Thunder, latest_version > 0 and 1 or 2) and #(self:getChainedEnemies(player)) > 1 then value = value - 25 end
