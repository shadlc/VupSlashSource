...
local function getIdToCard(self, cards, limit)
	limit = limit or 0
	local tocard = {}
	for _, card_id in ipairs(cards) do
		local card = sgs.Sanguosha:getCard(card_id)
		if limit > 0 and #tocard < limit then
...
local function GuanXing(self, cards, up_limit, down_limit)
	local up, bottom = {}, {}
	...
	--if down_limit == -1 then
	--	down_limit = #cards
	--end
	--if up_limit + down_limit < #cards then
	--	down_limit = #cards - up_limit
	--end
	down_limit = #cards		--GuanXing已暂时让置底上限失效（因为原函数是事先把所有牌置底然后挑选置顶），拖底上限部分在 SmartAI:askForGuanxing 处处理

	bottom = getIdToCard(self, cards, down_limit)
	...
	if not self.player:containsTrick("YanxiaoCard") then
		...
		for judge_count, need_judge in ipairs(judge) do
			...
			for index, for_judge in ipairs(bottom) do
				...
				if judge_str == suit and #up < up_limit then
		...
		if lightning_index then
			for index, for_judge in ipairs(bottom) do
				...
				if not (for_judge:getNumber() >= 2 and cardNumber <= 9 and cardSuit == "spade") and #up < up_limit then
		...
		if not self_has_judged and #judge > 0 and #cards <= down_limit then
			return {}, cards
		end
		...
		if conflict then
			if sname == "tuqi" then
				if self.player:getPile("retinue"):length() > 0 and self.player:getPile("retinue"):length() <= 2 then
					if #bottom > 0 and #up < up_limit then
			...
			elseif sname == "luoshen" then
				if #bottom == 0 then
					...
					for i = 1, #bottom do
						if bottom[i - count]:isBlack() and #up < up_limit then
	...
	if drawCards > 0 and #bottom > 0 then
		...
		for index, gcard in ipairs(bottom) do
			...
			if insert and #up < up_limit then
		...
		if #bottom > 0 and drawCards > 0 then
			...
			for i = 1, #bottom do
				...
				for index, card in ipairs(bottom) do
					if card:getEffectiveId() == c:getEffectiveId() and #up < up_limit then
...
local function XinZhan(self, cards)
	...
	bottom = getIdToCard(self, cards, 0)
	for judge_count, need_judge in ipairs(judge) do
		local index = 1
...
function SmartAI:askForGuanxing(cards, guanxing_type, up_limit, down_limit)
	--KOF模式--
	...
	if guanxing_type == sgs.Room_GuanxingBothSides then
		local up, bottom = GuanXing(self, cards, up_limit, -1)	--GuanXing已暂时让置底上限失效（因为原函数是事先把所有牌置底然后挑选置顶）
		while #bottom > down_limit and down_limit ~= -1 do	--因此此处将多出的置底部分塞入置顶的后面
			table.insert(up, table.remove(bottom))
		end
		return up, bottom
	elseif guanxing_type == sgs.Room_GuanxingUpOnly then
		--return XinZhan(self, cards)	--弃用心战函数
		
		local up, bottom = GuanXing(self, cards, up_limit, -1)	--GuanXing已暂时让置底上限失效（因为原函数是事先把所有牌置底然后挑选置顶）
		while #bottom > 0 do	--因此此处将多出的置底部分塞入置顶的后面
			table.insert(up, table.remove(bottom))
		end
		return up, bottom
